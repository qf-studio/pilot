...
func TestHandleLogin_Apple(t *testing.T) {
	...
	// Apple requires PKCE
	if !strings.Contains(loc, "code_challenge=") {
		t.Errorf("Location %q missing PKCE code_challenge for Apple", loc)
	}
	if !strings.Contains(loc, "code_challenge_method=S256") {
		t.Errorf("Location %q missing code_challenge_method=S256 for Apple", loc)
	}
	if !strings.Contains(loc, "response_mode=form_post") {
		t.Errorf("Location %q missing response_mode=form_post for Apple", loc)
	}
	...
	for _, s := range h.pending {
		if s.codeVerifier == "" {
			t.Error("Apple pending state missing codeVerifier")
		}
	}
}
...
func TestOAuthResolvedScopes_Apple(t *testing.T) {
	...
	if !found {
		t.Errorf("expected openid in apple default scopes, got %v", scopes)
	}
}
