...
func (f *FeedbackLoop) generateBody(prState *PRState, failureType FailureType, failedChecks []string, logs string, iteration int, knownPatterns []*memory.CrossPattern) string {
	...
	// GH-5336: do NOT emit "Depends on: #<original>" here. The original issue
	// stays OPEN (labeled pilot-superseded, never closed — see
	// notifyExternalClose around controller.go:3402) once this fix issue
	// takes over, and the SDK poller's hasPendingDependencies treats any
	// "Depends on:" line as an unmet-dependency gate against whatever it
	// names, open or not. That combination permanently blocked this very fix
	// issue from ever dispatching (#5321→#5322, #5324→#5325, silent for 2+
	// hours on 2026-09-06 until the originals were closed by hand). The
	// source issue number is instead carried in the autopilot-meta comment
	// below (source:N), which the poller does not parse as a dependency —
	// owner_death.go's parseFixIssueSource reads it from there.
...
// `<!-- autopilot-meta ... -->` comment, shared by CreateFailureIssue and
// CreateReviewIssue so both fix-issue flavors carry the same recovery
// metadata. branch/pr/iteration are always present (callers only invoke this
// once BranchName is confirmed non-empty); source and sha are included only
// when the corresponding PRState field is populated.
//
// GH-5348: sha:<full commit SHA> is the fix's own belt-and-suspenders on top
// of branch: — pilot-console #275 showed that reusing just the branch name is
// not enough. When the original PR's branch is deleted (e.g. on PR close),
// the executor has no way to recover the original diff from the branch name
// alone; it silently rebuilt the "fix" from main, discarding the real
// commits. The full SHA (not the 7-char prefix already in the human-readable
// Context section above) lets the worktree be recreated from the exact
// original commit — `git fetch origin <sha>` — even with the branch gone.
func autopilotMetaFields(prState *PRState, iteration int) string {
	meta := fmt.Sprintf("branch:%s pr:%d iteration:%d", prState.BranchName, prState.PRNumber, iteration)
	if prState.IssueNumber > 0 {
		meta += fmt.Sprintf(" source:%d", prState.IssueNumber)
	}
	if prState.HeadSHA != "" {
		meta += fmt.Sprintf(" sha:%s", prState.HeadSHA)
	}
	return meta
}
...
func (f *FeedbackLoop) CreateReviewIssue(ctx context.Context, prState *PRState, reviews []*github.PullRequestReview, comments []*github.PRReviewComment, iteration int) (int, error) {
	...
	// GH-5336: no "Depends on: #<original>" here either — see the matching
	// comment in generateBody for why that deadlocks the SDK poller against
	// the open, pilot-superseded original. source:N in the meta comment below
	// carries the same information without tripping hasPendingDependencies.
