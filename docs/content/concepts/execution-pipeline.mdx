...
- The prompt engages the Navigator loop contract: PLAN → EXECUTE →
...
only. No project context injection, no Navigator loop contract, no
...
**Backends**: Claude Code is the default; OpenCode and Qwen are
supported. Backend selection affects timeouts and signal parsing — see
...
DOCUMENT phase of the Navigator loop (originating issue: GH-1388).
