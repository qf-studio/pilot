...
Pilot respects issue dependencies declared in the issue body. A dependency
must be written **inline** as one of these phrases followed by a numeric
issue reference:
...
Recognized phrases (case-insensitive): `Depends on: #N`, `Blocked by: #N`,
`Requires: #N`. The `#N` must follow the phrase on the same line — list
items under a `## Depends on` header or prose like `Depends on: the auth
scaffold` are **not** detected, and the issue dispatches immediately.
