...
- `DEVELOPMENT-README.md` — index, read this first
- `tasks/` — implementation plans (archive when done)
- `system/` — architecture docs
```

For interactive planning workflows on top of this structure, see the
[Navigator plugin](https://github.com/alekspetrov/navigator) for Claude Code.
...
Task implementation plans. Each task gets a markdown file with:
...
| Context markers missing | First session | Markers are created at session checkpoints; they accumulate as tasks run |
