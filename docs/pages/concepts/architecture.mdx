...
Trivial tasks skip Navigator entirely. Simple through complex tasks get full Navigator context.
...
Navigator is a Claude Code plugin that provides structured codebase context. When Pilot detects a `.agent/` directory in the project, it injects `Start my Navigator session.` into the prompt.
...
├────────────────────────┤
│ "Start my Navigator    │ ← Triggers Navigator plugin
│  session."             │
├────────────────────────┤
...
Navigator phases detected in the execution stream:

| Phase | Signal | Description |
|-------|--------|-------------|
| `navigator` | `Navigator Session Started` | Plugin loaded, docs indexed |
...
Navigator integration is Pilot's core value proposition. The prompt injection (`Start my Navigator session.`) must never be removed — it's what gives Pilot deep codebase understanding rather than generic code generation.
</Callout>
