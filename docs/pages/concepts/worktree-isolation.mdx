...
On Pilot startup, orphaned worktrees are detected and removed:

1. **Scan**: Check `/tmp/pilot-worktree-*` directories
2. **Validate**: Verify each directory is a valid worktree
3. **Clean**: Remove orphaned directories and stale git references
4. **Prune**: Run `git worktree prune` to clean .git/worktrees/

This handles cases where Pilot crashed before normal cleanup could run.
