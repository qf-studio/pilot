...
| Simple | `claude-opus-4-6` | $5 / $25 per 1M tokens | Full capability at Sonnet pricing |
| Medium | `claude-opus-4-6` | $5 / $25 per 1M tokens | Standard feature work needs solid reasoning |
| Complex | `claude-opus-4-6` | $5 / $25 per 1M tokens | Architectural work gets the most capable model |

<Callout type="info">
Opus 4.6 replaced Sonnet as the default for simple/medium/complex tasks. Same pricing ($5/$25 per 1M tokens) but higher capability. Epic tasks are decomposed before execution, so each subtask routes independently.
</Callout>
...
| Complex | 60 minutes | Architectural changes with full Navigator workflow |
...
|----------|---------|--------|--------|---------|------|
| Navigator injection | Skip | Full | Full | Full | N/A (decomposed) |
...
**Navigator injection**: Trivial tasks skip `Start my Navigator session.` to avoid ~30s overhead on a task that takes 60s total.
...
executor:
  model_routing:
    ...
    simple: "claude-opus-4-6"      # full capability
...
└──────────┬──────────┘
     ...
     ┌─────┴──────────────┐
     │ ModelRouter         │
     │  .SelectModel()     │──→ "claude-haiku" or "claude-opus-4-6"
