...
### 2. Navigator: deep codebase context

This is Pilot's core differentiator. [Navigator](/navigator) is a Claude Code plugin that provides deep codebase understanding through strategic documentation loading. It reduces token usage by **92%** (from 150k to 12k tokens per session) while giving Pilot knowledge of your:
...
Without Navigator, AI generates generic code. With Navigator, it generates code that follows your established conventions. The difference between code that *works* and code that *belongs*.
...
| Complexity | Model | Navigator | Cost | Example |
|------------|-------|-----------|------|---------|
...
| **Codebase context** | ✅ Navigator (deep) | ⚠️ File-level | ⚠️ File-level | ⚠️ Proprietary | ⚠️ Manual CLAUDE.md |
...
| **Model routing** | ✅ Haiku + Opus | ❌ Fixed | ❌ Fixed | ❌ Fixed | ⚠️ Manual |
