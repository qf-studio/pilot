...
**Status**: 🔴 DAEMON STOPPED 2026-07-17 01:27Z — **split-brain incident [#4393](https://github.com/qf-studio/pilot/issues/4393)**: config.yaml:388 absolute path `/Users/aleks.petrov/.pilot/data` was NOT covered by the shims (only `/Users/.../Projects` was) → box daemon auto-created a shadow ledger there (root volume) and wrote 92 executions 23:56–01:24Z invisible to board/metrics/claims. Watch stopped it (kill pre-authorized). **Before restart**: shim `/Users/aleks.petrov/.pilot → /var/lib/pilot/pilot-home` (or rewrite config path, operator consent) + merge shadow rows into canonical DB (executions, execution_claims max-generation wins, autopilot_pr_state) — see #4393. Pitfall: `.agent/knowledge/memories/pitfalls/absolute-state-paths-bypass-cutover-shim.md`. — Cutover itself was 2026-07-16 22:32Z (tmux `pilot`, v2.241.1, wrapper /home/ec2-user/start-pilot.sh). Local ~/.pilot FROZEN (rollback tar ~/pilot-state-rollback-20260716.tgz, 7 days). NEVER `pilot start` locally again without the rollback procedure. Decisions (founder, 2026-07-16 ~21:05Z): **t3.xlarge (16GB)** · **Claude subscription login** (device flow on box) · **Created**: 2026-07-16 ~21:00Z
