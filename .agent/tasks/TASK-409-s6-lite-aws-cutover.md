...
**Status**: 🟢 RESOLVED + RESTARTED 2026-07-17 09:33Z — **split-brain incident [#4393](https://github.com/qf-studio/pilot/issues/4393)** closed out: `/Users/aleks.petrov/.pilot → /var/lib/pilot/pilot-home` symlink created 07:53Z (mirrors the existing `/Users/.../Projects` repo shim; `config.yaml:388`'s `/Users/aleks.petrov/.pilot/data` path kept **verbatim** — no config rewrite). Shadow ledger (95 executions, 23:56–01:24Z) merged into canonical DB (`pilot.db.pre-4393-merge.bak` is the pre-merge snapshot) and the shadow tree archived to `/Users/aleks.petrov/.pilot.shadow-archived-20260717/`. Daemon restarted 09:33Z (pid confirmed) — verified via `/proc/<pid>/fd`: fd 8/9/10 (`pilot.db`, `-wal`, `-shm`) all resolve to `/var/lib/pilot/pilot-home/data/*`, zero shadow-path FDs. **GH-4393-2 reconciliation re-verified 2026-07-17 by row-count audit against the three DBs still on disk**: `executions` 3445(pre-merge)+95(shadow)=3540 rows ≤ merge cutoff in canonical (exact union, PK is a UUID so no collision risk); `execution_claims` 43+94=137 rows ≤ cutoff, **zero** `task_id+project_path` overlap between the two DBs (both sides switched from the laptop-path key `/Users/aleks.petrov/Projects/...` to the box-resolved key `/var/lib/pilot/repos/...` at the same cutover instant, 2026-07-16 23:56Z, and stayed consistent since — so "max-generation-wins" had nothing to arbitrate, the union was already correct); `execution_events` (autoincrement PK) re-keyed past canonical's existing max id (7619→8620) rather than reusing shadow's 1–375 range, avoiding a silent PK-collision drop — spot-checked all 86 distinct shadow `execution_id` FKs present in canonical with matching row counts; `autopilot_pr_state` count unchanged (18) — expected, rows are deleted on completion by design (`internal/autopilot/state_store.go:704`), not evidence of a merge gap; the 3 still-open canary PRs from the shadow window (#87/#90/#94) are present in canonical with fresh post-restart state, and PR #4388 is correctly absent (merged 07:54Z, row cleaned up). **Correction**: the issue's fix-item #2 lists a table `instance_events` for merge — no such table exists in the schema (`.tables` on `pilot.db`); this looks like a naming slip for `execution_events`, which is confirmed merged above. Pitfall: `.agent/knowledge/memories/pitfalls/absolute-state-paths-bypass-cutover-shim.md`. **GH-4393-3 shipped 2026-07-17**: startup banner now logs the resolved DB path — `resolvedDBPath()` (`cmd/pilot/main.go`) takes `Memory.Path` (or the `~/.pilot/data` fallback), makes it absolute, and evaluates symlinks (so a shim like the one in this cutover resolves to its real target), then joins `pilot.db`; `banner.StartupBanner`/`StartupTelegram` print it and both headless startup paths `slog.Info` it under component `start` (`db_path` field) so this class of drift shows up in logs even without a TTY. **GH-4393-4 shipped 2026-07-17**: `acquireDaemonLock` (`cmd/pilot/main.go`) now calls `checkShadowInit(dir)` before `singleton.Acquire`, which is the actual `os.MkdirAll` call site — if the configured state dir doesn't exist yet AND the well-known default (`~/.pilot/data`) already has a `pilot.lock` or `pilot.db` (evidence this host has run pilot before) at a *different* path, the daemon now refuses to start with an `errShadowInit` explaining the drift and how to override (`mkdir -p` the configured dir to confirm a genuine second install). A true first run (nothing anywhere yet) and a dir that already exists are both no-ops. This closes the silent-empty-init half of the incident the banner logging (GH-4393-3) only made *visible*. Remaining #4393 scope: `pilot-board-remote` FD assertion — not blocking. — Cutover itself was 2026-07-16 22:32Z (tmux `pilot`, v2.241.1, wrapper /home/ec2-user/start-pilot.sh). Local ~/.pilot FROZEN (rollback tar ~/pilot-state-rollback-20260716.tgz, 7 days). NEVER `pilot start` locally again without the rollback procedure. Decisions (founder, 2026-07-16 ~21:05Z): **t3.xlarge (16GB)** · **Claude subscription login** (device flow on box) · **Created**: 2026-07-16 ~21:00Z
