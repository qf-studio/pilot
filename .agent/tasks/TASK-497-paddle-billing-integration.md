...
## Legs (issues) — dispatch order L1 → L2 → (L3 ∥ L4) → L5b (#306) → L5a (#308) → L6 → L7 (go-live, Navigator + founder, not a Pilot issue)

Issue-body rules at dispatch: H2 headers `## Problem / ## Fix / ## Acceptance`, inline `Depends on: #N`, backticked paths only if present on the target repo's `origin/main` (run the path check with NO exclusions — even a path you are telling Pilot not to create trips the base-presence gate, pilot#5437 on 09-12), no migration version literals, `no-decompose` label on EVERY one-PR spec (without it the executor may classify the issue as an epic: planning mode, 30-minute timeout, Sonnet routing — pilot#5437 timed out twice on 09-12), mutation command in acceptance.
...
### L4 — console: customer portal session endpoint — ✅ [console#305](https://github.com/qf-studio/pilot-console/issues/305) → **PR#307 merged 12:12Z, reviewed APPROVE-w-notes** (3/3 mutation pins run by the reviewer in a worktree; note: no evidence paste because the box predates the gate). #306 picked up 12:13Z — the `Depends on:` chain released it within a minute.
- **Deltas vs D5 found while authoring**: `billing.Org` has no subscription id → L4 adds `BillingSubscriptionID` to `Org` + `billingOrgAdapter.OrgByUser` · **409 also when the org has a customer but no subscription** (abandoned checkout leaves a `ctm_` with no `sub_`; the UI redirects its placeholder tab to whichever URL it picked, so an empty URL must never be a 200) · 502 when Paddle's `urls.subscriptions` lacks the org's id · three named mutation tests (`TestPortalSessionRequestCarriesSubscriptionID`, `TestPortalSessionNoStore`, `TestPortalSessionRouteMounted`).
...
### L5 — console: billing enforcement sweep (past_due grace → suspend, active → resume, resync) — ✅ **SHIPPED 2026-09-14** — part 2 [console#306](https://github.com/qf-studio/pilot-console/issues/306) → **PR#309 merged 09-13 12:33Z** · part 1 [console#308](https://github.com/qf-studio/pilot-console/issues/308)/[#310](https://github.com/qf-studio/pilot-console/issues/310) → **PR#312 merged 09-14 11:36Z** (`f815bba`). Both reviewed APPROVE-w-notes. Enforcement is **wired but dormant**: `PILOT_CONSOLE_BILLING_ENFORCE_ENABLED` defaults false, so the sweep logs would-be suspends/resumes at Warn and writes nothing until an operator flips it.
- **Status 09-14 11:36Z**: part 1 **→ console#310 → PR#312 MERGED** (`f815bba`; held 18 h in autopilot `awaiting_approval` — 708 code additions > the 500 threshold, test additions excluded — founder approved 09-14). Reviewed APPROVE-w-notes. Tree ≡ 0e704c0; 4/4 mutation pins pasted (the two Postgres-backed ones genuinely failed under mutation — the box has a DB for console tests). Run 6 of #308 had also "completed" by opening PR#311 on the same preserved commit with no evidence → closed in favour of PR#312, which closes #308 + #310. Executor follow-up pilot#5445 → **PR#5446 MERGED 09-14 11:36Z** (`0396df0a`; held on a title-scope divergence, executor vs dispatcher — my wording, not a defect). Task doc archived. **Not on the box yet — needs the next train.** Review notes on PR#312 (not filed, flag is off): NULL `billing_status_event_at` + past_due → immediate suspend; grace-warn map never resets; dry-run Warn every tick; B7-slept rows are not suspended (roadmap item 10).
- **Status 09-13 evening**: part 2 **✅ console#306 → PR#309 merged 12:33Z, reviewed APPROVE-w-notes** (mutation evidence pasted by Pilot: 2/2). Part 1 **console#308 ran 5× and never produced a PR** (box daemon.log: runs 1+3 = 1 h `complex` timeout on Sonnet; runs 2+4 = `unknown: exit status 1` mid-stream; run 5 = finished without committing → daemon **auto-preserved the dirty worktree as 0e704c0 on pilot/GH-308**). Navigator verified 0e704c0: build/vet/`go test ./...`/lint/wire-contract gate all green, every named test present — the spec is fully implemented, only the PR + evidence paste are missing. Pinned under remote ref preserve/GH-308-wip-0e704c0 (plain `git push` from a fresh run would otherwise collide with it). `pilot` label removed from #308 (run 6 had already started 16:22Z from origin/main — doomed by the non-fast-forward push). **Continuation → [console#310](https://github.com/qf-studio/pilot-console/issues/310)**: cherry-pick 0e704c0, run the 4 mutation pins, paste evidence, `Closes #308`. Executor defect filed on pilot (re-pick after auto-preserve restarts from scratch instead of escalating).
- **Split (Navigator decision 09-13)**: part 2 = operator tools → [console#306](https://github.com/qf-studio/pilot-console/issues/306) (`consolectl billing set-status` / `resync`, both through `ApplyBillingWebhookEvent`; `GetSubscription` on `PaddleClient`; `GetOrgByID`) · part 1 = sweep + reconciler lifecycle → [console#308](https://github.com/qf-studio/pilot-console/issues/308). Chain via inline `Depends on:` so no two PRs edit the same files concurrently: **#305 (L4) → #306 → #308**. Both `pilot` + `no-decompose`; the poller holds each until its dependency closes and the base-presence gate checks the merge landed.
- **D4 prerequisite check FAILED (found while authoring)**: the reconciler treats `suspended` as out of scope — `decide` returns `actionNone` for `desired=suspended`, `sync` never writes `observed=suspended`, no store writer flips to/from `DesiredSuspended`. A sweep that only set `desired_state` would leave EC2 running. #308 therefore adds the lifecycle: `decide` suspended+running→`actionStop`; `sync` live-stopped under suspended → `ObservedSuspended`; resume writer flips desired→running **and** observed suspended→stopped so `actionStart` is reachable; resume only when the latest `billing_suspended`/`operator_suspended` journal row is `billing_suspended`.
- Other #308 specifics: `SetBillingStates` setter (SetReplacer idiom) with an orgs-store adapter in `main.go`; `ListBillingStates` on orgs (status ≠ none); `Organization.BillingStatusEventAt`; env `PILOT_CONSOLE_BILLING_ENFORCE_ENABLED` (default off, dry-run Warn) + `PILOT_CONSOLE_BILLING_PAST_DUE_GRACE` (72h); `handleConfig` adds `past_due_grace_hours` + `past_due_since` only when past_due; no metrics/alert sink in this leg. #306 specific: the apply path writes `billing_subscription_id` from the event unconditionally, so operator events must carry the org's current ids.
...
### L6 — sandbox smoke SOP + fixtures + docs — 🟡 **FIRST RUN DONE 2026-09-14, one step left**
- **SOP written**: `sops/billing/paddle-sandbox-smoke.md` (corrected in place after the run).
- **Proven live against the sandbox**: checkout-session → real transaction + customer persisted (L1) · notification destination `ntfset_01m2fyef5n3a7sfaxhd9r1260q` + simulation → **3 real signed webhooks verified and ledgered insert-first**, unresolvable events recorded not dropped (L2) · portal 409 + `no-store` (L4) · `consolectl billing set-status`/`resync` through the ledger with both guards (L5 part 2) · config endpoint past-due fields (L5 part 1, partial).
- **Defect found → [console#313](https://github.com/qf-studio/pilot-console/issues/313) → PR#314 merged 13:02Z (`3a7fc95`), reviewed APPROVE**: `past_due_grace_hours` truncated to whole hours and vanished below 1 h (`int(Hours())` + `omitempty`). Fix ships `past_due_grace_seconds` on a **separate response type**, so presence is decided by which type is marshalled rather than a zero value — structurally stronger than the drop-omitempty fix the issue asked for. **Re-verified live on the rebuilt stack**: 72h/90m/1m/30s → 259200/5400/60/30, never absent; non-past_due org still gets exactly the three base fields. Bug found by the smoke, fixed and closed by the same smoke inside 20 minutes.
- **CARD PAYMENT DONE 14:47Z** (founder clicked through on the tunnel origin): real sandbox checkout with `4242…` → `subscription.created` + `subscription.activated` + `transaction.completed` **all resolved to the demo org** (`billing: applied webhook billing transition`), subscription `sub_01m2g66jh5drthr7naxpkpthvn` bound, org `active`, UI chip flipped and Manage-subscription appeared. **The full L1→L2→L3 happy path is now proven end to end with a real card**, including the org-resolution leg the simulation could not reach.
- **SECOND DEFECT, founder-spotted from the screenshot**: the plan page advertises **$299 / month** while the catalog charges **USD 500** — hardcoded markup in the billing view, documented in its own comment as placeholder copy from the design screen. Harmless while checkout was off; now it quotes one price beside a button that charges another, **and Paddle's verification reviewers check on-site pricing against the live catalog, so it is an L7 go-live blocker**. Two legs: [console#315](https://github.com/qf-studio/pilot-console/issues/315) → **PR#316 merged 15:05Z (`65768a3`), reviewed APPROVE-w-notes, verified live** (real catalog → `price_amount:"50000"/USD/month/1`; bad price id → fields omitted, registration survives, one Warn — omit-don't-guess proven both ways); [ui#162](https://github.com/qf-studio/pilot-console-ui/issues/162) → **PR#163 merged, reviewed APPROVE-w-notes, verified live in the browser: the page now reads "Team $500.00 / month"** (EUR case asserts no `$` appears; absent fields render no price element; mock adapter reseeded to the real figures) — label had been added by hand at 16:10Z now that #316 has merged (it was held because a cross-repo `Depends on:` does not resolve — the poller matches issue numbers within one repo; that gating is manual across repos). **CON-5 billing-copy founder input is now closed.**
- **Left**: the enforcement sweep (needs staging with fleet on — not provable on the local stack).
...
- Pilot issues: L1 console#297 → PR#298 (merged) · L1 follow-up console#299 · L2 console#300 · L3 ui#156 → PR#157 (merged) · L3 follow-up ui#158 → PR#159 (superseded) → ui#160 · **L4 console#305** · **L5 part 2 console#306 · L5 part 1 console#308** (gated: #305 → #306 → #308) · executor evidence-gate side quest pilot#5435 → PR#5436 (closed) → #5437/#5438 → PR#5439/#5440 (merged) → #5441 (declined) → #5442

---

**Last Updated**: 2026-09-14 (rev 14: ui#162 → PR#163 merged, reviewed, verified live — price mismatch fully closed, CON-5 done; rev 13: console#315 → PR#316 merged, reviewed, verified live; ui#162 released; rev 12: card payment done — full happy path proven; price-mismatch defect → console#315 + ui#162; rev 11: console#313 → PR#314 merged + reviewed + re-verified live; rev 10: L6 first run — webhook chain proven live, console#313 filed, card leg + sweep still open; rev 9: L5 SHIPPED — PR#312 + PR#5446 merged 11:36Z; enforcement dormant behind the default-off flag; L6 is next and gated on founder inputs; rev 8: PR#312 + PR#5446 reviewed, both awaiting founder approval; PR#311 closed; rev 7: #306 merged+reviewed; #308 5× failed → 0e704c0 preserved + verified → continuation #310; rev 6: L4 merged + reviewed, #5443 reviewed, #306 running; rev 5: L5 split into #306 + #308, gated chain, reconciler-suspended gap recorded; rev 4: L4 dispatched as console#305 with three D5 deltas; pilot#5441 declined → #5442; rev 3: L7 go-live leg adapted from Paddle's prompt; D2 source-IP allowlist from `GET /ips`; rev 2: Paddle plugin installed; D4 → sweep + grace + resync; founder checklist shrunk)
