...
- **Left**: the card payment (Paddle overlay needs an https parent origin; one manual click-through finishes it) and the sweep itself (needs staging with fleet on — not provable on the local stack).
...
**Last Updated**: 2026-09-14 (rev 11: console#313 → PR#314 merged + reviewed + re-verified live; rev 10: L6 first run — webhook chain proven live, console#313 filed, card leg + sweep still open; rev 9: L5 SHIPPED — PR#312 + PR#5446 merged 11:36Z; enforcement dormant behind the default-off flag; L6 is next and gated on founder inputs; rev 8: PR#312 + PR#5446 reviewed, both awaiting founder approval; PR#311 closed; rev 7: #306 merged+reviewed; #308 5× failed → 0e704c0 preserved + verified → continuation #310; rev 6: L4 merged + reviewed, #5443 reviewed, #306 running; rev 5: L5 split into #306 + #308, gated chain, reconciler-suspended gap recorded; rev 4: L4 dispatched as console#305 with three D5 deltas; pilot#5441 declined → #5442; rev 3: L7 go-live leg adapted from Paddle's prompt; D2 source-IP allowlist from `GET /ips`; rev 2: Paddle plugin installed; D4 → sweep + grace + resync; founder checklist shrunk)
