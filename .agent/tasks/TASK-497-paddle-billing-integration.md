...
- **SECOND DEFECT, founder-spotted from the screenshot**: the plan page advertises **$299 / month** while the catalog charges **USD 500** — hardcoded markup in the billing view, documented in its own comment as placeholder copy from the design screen. Harmless while checkout was off; now it quotes one price beside a button that charges another, **and Paddle's verification reviewers check on-site pricing against the live catalog, so it is an L7 go-live blocker**. Two legs: [console#315](https://github.com/qf-studio/pilot-console/issues/315) serves the catalog price on the config endpoint (startup lookup, cached, omit-don't-guess on failure) → running; [ui#162](https://github.com/qf-studio/pilot-console-ui/issues/162) renders it via `Intl.NumberFormat` and renders nothing when absent — **label held** until the console leg merges (cross-repo `Depends on:` does not resolve; the poller matches issue numbers within one repo). This closes the CON-5 billing-copy founder input.
...
**Last Updated**: 2026-09-14 (rev 12: card payment done — full happy path proven; price-mismatch defect → console#315 + ui#162; rev 11: console#313 → PR#314 merged + reviewed + re-verified live; rev 10: L6 first run — webhook chain proven live, console#313 filed, card leg + sweep still open; rev 9: L5 SHIPPED — PR#312 + PR#5446 merged 11:36Z; enforcement dormant behind the default-off flag; L6 is next and gated on founder inputs; rev 8: PR#312 + PR#5446 reviewed, both awaiting founder approval; PR#311 closed; rev 7: #306 merged+reviewed; #308 5× failed → 0e704c0 preserved + verified → continuation #310; rev 6: L4 merged + reviewed, #5443 reviewed, #306 running; rev 5: L5 split into #306 + #308, gated chain, reconciler-suspended gap recorded; rev 4: L4 dispatched as console#305 with three D5 deltas; pilot#5441 declined → #5442; rev 3: L7 go-live leg adapted from Paddle's prompt; D2 source-IP allowlist from `GET /ips`; rev 2: Paddle plugin installed; D4 → sweep + grace + resync; founder checklist shrunk)
