...
- **GH-3917 defect fix** (2026-07-06): `newGithubSDKPollerDef` was passing `ghCfg.Token` verbatim into the SDK client/config instead of running the config→env→gh-CLI resolution chain — with `token: ""` in config, every poll 401'd silently while startup logged "polling enabled" (this outage briefly took down the whole qf-studio/pilot dispatch pipeline, since 4b gives the SDK poller exclusive ownership of the default repo). Fixed in `cmd/pilot/poller_github.go`: resolve via `resolveGitHubToken`, fail loud via `verifySDKGithubToken` (one `GetAuthenticatedUser` call, disables the poller on a confirmed 401), tag this poller's own log lines `component=github-sdk-poller`. **Known gap, not fixed here:** `sdkcore.PollerDeps` (studio-sdk v0.28.1) has no `Logger` field and `Adapter.NewPoller` doesn't expose the lower-level `WithPollerLogger` option, so the *internal* poller loop's own log lines (e.g. `poller.go:682` "Failed to fetch issues") still go through `slog.Default()` untagged — that incident's actual untagged 401 spam. Threading a logger through requires a studio-sdk change (add `PollerDeps.Logger`, wire it into `Adapter.NewPoller`'s `PollerOption`s) plus a version bump here; out of scope for a pilot-only PR — fold into 4d.6 cleanup or its own upstream task.
