...
**Status**: 🟡 **All three legs merged 08-18 + post-merge reviewed 08-19 — but Leg A's merged fix is a FUNCTIONAL NO-OP.** Leg A #4965 → PR#4978 **APPROVE-w-defects, CRITICAL**: `$teamId: String!` declared in an `ID` position; live api.linear.app rejects the exact merged document `GRAPHQL_VALIDATION_FAILED` pre-auth, so the #4884 duplicate-label symptom persists unchanged. One-token fix filed as **[pilot#4985](https://github.com/qf-studio/pilot/issues/4985)** (`ID!` + pin the declaration in the test — the httptest mock validates no GraphQL, which is how this shipped green; pitfall memory `graphql-mock-tests-cannot-catch-schema-validation`). Leg B #4966 → PR#4979 APPROVE-w-notes (exported `StripIssuePrefix` single-sources #4899's widened grammar; `(#N)` train suffix structurally safe). Leg C #4967 → PR#4980 APPROVE-w-notes (all four copies consolidated + actually called on every slack/discord/telegram path; note: telegram's 7 extra v1 markers silently dropped — net improvement, undocumented). **Close this task when #4985 merges.**
**Created**: 2026-08-17
