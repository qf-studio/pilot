...
**Status**: 🚀 ACTIVE — 11 of 17 legs MERGED + REVIEWED, all APPROVE (console PR#119/121/123/125 · ui PR#51/53/55/57/59/61/63). Console chain DONE except founder-gated CON-5. t7 dispatched: UI-8 connections (ui#64). Remaining: UI-9..12 + CON-5. Side-filed: pilot#4869 (journal-close on external merge — history rows stuck at awaiting_approval).
**Created**: 2026-08-14
...
| UI-8 | Connections v1 | M | — (slack leg) | 🚀 [#64](https://github.com/qf-studio/pilot-console-ui/issues/64) |
...
- UI-8: ui#64 (2026-08-14) — SECOND adapter drift found while authoring (same class as UI-7's): `upsertConnection` PUTs flat fields but merged console wants `{label,secret,config}` (every save 400s today) · `testConnection` types `{ok,reason?}` but the route always-200s `{valid,error?}` · jira missing `project_key`, linear missing `team_key` (syncingest-read additive config). Body orders the re-anchor + connections-v1 restyle (flat list w/ Work sources/Model/Notifications groups · GitHub detail w/ SSM trust copy · jira/linear forms, test-before-save). 422-persists-nothing semantics quoted
