...
- **⚠️ ACTIVATION GAP → [pilot#5019](https://github.com/qf-studio/pilot/issues/5019)**: `SetContractContentFetcher` never wired by any PR (decomposition gap; FEATURE-MATRIX admits it) — any project opting in today hard-fails ALL contract tasks with `fetch_error`. **Do NOT set `contract_dependencies` on any project until #5019's legs ship.** Then operator step: add ui's `contract_files` incl. `src/lib/api/types.ts`.
