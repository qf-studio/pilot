...
- **⚠️ ACTIVATION GAP → [pilot#5019](https://github.com/qf-studio/pilot/issues/5019)**: `SetContractContentFetcher` never wired by any PR (decomposition gap; FEATURE-MATRIX admits it) — any project opting in today hard-fails ALL contract tasks with `fetch_error`. #5019 also carries the #5014 docblock fix, #5016 diff-error fail-open fix, empty-fields LLM-call guard. **Do NOT set `contract_dependencies` on any project until #5019 ships.** Then operator step: add ui's `contract_files` incl. `src/lib/api/types.ts`.
