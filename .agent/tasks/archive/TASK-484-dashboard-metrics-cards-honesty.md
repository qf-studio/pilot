...
**Status**: 🚀 Dispatched to Pilot (spec amended post risk-review — issue #5192 body is authoritative)
**Last Updated**: 2026-08-24

> **Risk-review amendments (2026-08-24)**: issue body now carries hard constraints — title ≤17 / detail ≤19 visible chars (grom truncation budgets at cw=23); band flooring via series pre-processing only (vendored grom `BrailleStacked` untouched); `buildStatCard*` signatures frozen; goldens to update: `TestFullDashboardRender_WidthInvariants` (`"╭─ queue depth "`) + `TestRenderTokenCard_CacheBreakdown` (`"90.0K cached"`). Window label moves to card TITLE (`tokens · all`), not detail. Verdict: legs A/B LOW-MEDIUM, C LOW; modelPricing pure, single caller, execution-time only — historical `estimated_cost_usd` rows never recomputed.
...
- Pilot issue: https://github.com/qf-studio/pilot/issues/5192
