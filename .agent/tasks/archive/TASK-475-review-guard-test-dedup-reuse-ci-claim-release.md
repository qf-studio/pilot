...
**Status**: ✅ CLOSED 2026-08-13 — merged 08-12 12:55Z ([pilot#4860](https://github.com/qf-studio/pilot/issues/4860) → **PR#4862**); **post-merge review HOLDS, zero defects** ([verdict](https://github.com/qf-studio/pilot/pull/4862#issuecomment-5279310929)): all three spec items faithful; the exact PR#4858 `(0,nil)` mutant is killed by the new seeded-claim test alone (all 4 load-bearing assertions); secondary mutants (release dropped, reuse neutered) killed; GH-4852 scenarios preserved — reuse strengthens claim-gated decline and cannot race a release; CI-path release covers all exits incl. PR#4861 SHA-change re-entry. Ledger notes (all pre-existing shapes): N1 vacuous mid-test assertion gh4860_test.go:264 (kill rests on the retry leg) · N2 reuse can designate a human-gated owner via the unhealth-checked TerminalLabel branch · N3 triple-fault release race, review path has no title-search self-heal · N4 `RecordSpawnedFixIssue` failure still poisons the key permanently. Spec's optional nits (rearmed-alert gate, alert double-fire dedup) still open. **LIVE in production since 08-12 14:18Z** — the v2.259.1 self-upgrade hot restart DID take effect (`upgrade verified complete … via "hot restart"` in daemon.log; the "disk≠process" reading was an uptime-heuristic misdiagnosis — `syscall.Exec` preserves PID/etime).
**Created**: 2026-08-12
**Assignee**: Pilot
