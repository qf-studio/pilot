...
**Status**: ✅ **COMPLETE + ARCHIVED 2026-08-19 — all three legs merged + reviewed.** Parser [pilot#4963](https://github.com/qf-studio/pilot/issues/4963) → PR#4975 (APPROVE-w-notes; spec's G5–G7 `-p` assumption was wrong, implementation corrected it). D5 env bypass [pilot#4968](https://github.com/qf-studio/pilot/issues/4968) → PR#4981 (APPROVE-w-notes; no fail-open path). Residuals [pilot#4986](https://github.com/qf-studio/pilot/issues/4986) (D2 graphql+fields deny + `--hostname` argv twin) → **PR#4990 merged 08-19 after pre-merge review APPROVE** — graphql scoping verified identical to cli/cli's `RequestPath == "graphql"`; one non-exploitable hardening nit left on the PR (empty `--hostname` gates on presence not value; gh's own validator makes it unreachable). #4905 (lkshrk) can still land GET+fields on top whenever the contributor returns.
**Created**: 2026-08-17
