...
**Status**: ✅ **COMPLETE 2026-08-19 — merged + post-merge reviewed APPROVE-w-notes.** [pilot#4964](https://github.com/qf-studio/pilot/issues/4964) → PR#4977 merged 08-18. Review verified full spec conformance: strict `no_op:true`+mandatory-reason evidence (never the bare mandatory exit signal), preserve-before-classify at all three insertion points, latent post-retry ordering bug fixed, ghost-SHA gated on the exact clean-no-commit const, tests drive real `Runner.Execute()` against real git repos. Notes only (unreachable defensive post-retry check; sticky `exitSignalNoOp` across smart retries — mislabeled reason at worst, no lost work). Deliberate behavior change locked in: dirty-tree pre-retry now auto-preserves as WIP instead of attempting the GH-916 retry. → ARCHIVED.
**Created**: 2026-08-17
