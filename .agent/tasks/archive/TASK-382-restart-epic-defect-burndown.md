...
| D6 | `Blocked by: #N` not enforced at queue/dispatch time — gated task queued and executed while its blocker was still open; per-project FIFO masked it | #3759 (`Blocked by: #3754`) queued 16:20, ran while #3754 open until 16:40 | [#3789](https://github.com/qf-studio/pilot/issues/3789) ✅ **shipped via GH-3882/TASK-384** — `hasPendingDependencies` now resolves blockers against the already-fetched `fetchCandidates` slice (zero new API calls), fixing the `stress` 600s deadlock that killed 4 prior attempts (PRs #3802/#3822/#3824/#3835) |
