...
**Status**: ✅ **ALL 4 PHASES MERGED — Phase 4 merged 2026-08-10** ([#4823](https://github.com/qf-studio/pilot/issues/4823) → **PR#4828**, same-day dispatch-to-merge; pre-dispatch re-anchor found the first parity target already shipped via GH-4779 `ciFailureConclusions` — spec re-scoped, `approval_source: ""` overlay bug confirmed at `controller.go:806`). **Post-merge review of PR#4828 DONE 2026-08-11 — all 7 spec tasks faithful; 1 defect → [#4831](https://github.com/qf-studio/pilot/issues/4831)** (destructive-call gate Check 2 passes vacuously on macOS: BSD grep lacks `-P`, exit 2 swallowed by `|| true`; `--self-test` catches it but is wired nowhere — CI's Ubuntu leg is sound, so the authoritative gate holds at PR time). Review posted on the PR. **ARCHIVED 2026-08-11.** **Phase 3 ✅ MERGED 08-09** ([#4817](https://github.com/qf-studio/pilot/issues/4817) → PR#4818 closed on lint after manual conflict resolution → retry generation **PR#4821 merged 08-09**, v2.258.0 train). **Post-merge review of PR#4821 DONE 2026-08-10 — no defects.** All 8 spec tasks verified faithful, incl. the `notifyExternalClose` shadowing hazard (the `issue, err` fetch at :7337 survives un-shadowed to the gate) and `fetchIssueState`'s fail-open contract. Two notes, neither defect-worthy: the closed-issue skip also suppresses the `pilot-in-progress`/`pilot-failed` *removals* (cosmetic stale label on closed issues), and `gh4817_no_artifact_gate_test.go` is a source-text guard test (existing `githubFuncBody` pattern). Recovery-path observations from the lint cycle recorded in roadmap backlog: fix-issue #4820's truncated logs (preflight `reject_vague`-ed it; closed as orphan 08-10) and dual-armed recovery (fix-issue + retry chain both armed; GH-3806 `TerminalLabel` didn't designate an owner). Phase 2 ✅ (#4811 → PR#4812; finding → #4813, fixed via #4816). Phase 1 ✅ (#4796 → PR#4802). Phase 5 split to [TASK-460](TASK-460-delivery-evidence-false-success.md) — this task is done at Phase 4
**Created**: 2026-08-07
...
### Phase 4: Parity + enforcement — 🚧 DISPATCHED (#4823, 2026-08-10)
**Goal**: Make drift and bypass impossible rather than unlikely.

**Tasks**:
- [x] Shared table for "what counts as a failed check" consumed by both status mapping and evidence gathering; parity test fails on divergence (the #4790 fix generalized). **Already shipped pre-dispatch via GH-4779** (`ciFailureConclusions` @ `ci_monitor.go:667` + `TestCIMonitor_MapCheckStatus_FailureConclusionParity`) — dropped from the #4823 spec.
...
- Phase 3 issue: https://github.com/qf-studio/pilot/issues/4817 → PR#4818 (closed on lint after manual conflict resolution) → **PR#4821 merged 2026-08-09** (post-merge review 2026-08-10: no defects). Orphaned fix issue #4820 closed 08-10 (preflight `reject_vague` on truncated logs; retry chain delivered first)
- Phase 4 issue: https://github.com/qf-studio/pilot/issues/4823 (dispatched 2026-08-10; anchors re-verified on main @ `39881bba`)
...
**Phase 4 dispatched** (2026-08-10, #4823): pre-dispatch anchor verification on main @ `39881bba` found (1) the first parity target already shipped (GH-4779) — spec re-scoped to the evidence-derivation corpus test, the approval-source vocabulary triplication, the `approval_source: ""` overlay guard (confirmed still missing at `controller.go:806`), and the §9 reason-string targets; (2) minor line drift only (`applyApprovalDecision` now :3209; `scope_release.go:433`'s substring is now a named constant but routing is still `strings.Contains`). `isDuplicateTagError`/`isDuplicateReleaseError` explicitly fenced out (low-stakes idempotence drains). Tasks declared independent in the spec — incremental commits, no entangling.
