...
**Status**: ✅ **SHIPPED 2026-07-12, all 3 fixes on main; v2.238.1 tagged out-of-band (incident, mem-104).** Fixes 1+2 via PR #4221 (decomposer `len<=1` + title normalization incl. decomposed-parent path); fix 3 via PR #4223 (cross-task-id guard) + PR #4228 (`GetDecomposedChildren` parser); extension PR #4229 merged 2026-07-12 evening (reviewed + merged interactively — guard now at all 4 `HasCompletedExecution` sites via shared `decomposedChildrenAllComplete`). Delivery was rough: fix 3 was first dropped by the epic run (children closed "completed as part of" without it) → re-filed #4222, which the pre-fix daemon recursed into a single-child cascade (4222→4224→4225→4226/4227), with the Defect-A re-dispatch loop firing live on the parents. Triage: cascade parents closed manually, v2.238.1 cut so the daemon stops cascading. #4216 closed COMPLETED. Verify post-upgrade: no new single-child decompositions, no parent re-dispatch after children ship.
