...
**Status**: ✅ **COMPLETE + ARCHIVED 2026-08-19.** Leg A #4965 → PR#4978 was APPROVE-w-defects (CRITICAL: functional no-op — `$teamId: String!` in an `ID` position, live API rejects pre-auth; pitfall `graphql-mock-tests-cannot-catch-schema-validation`); the fix **#4985 → PR#4991 merged same morning, reviewed APPROVE** — one-token `ID!`, declaration-pinning test, mutation-tested during review (reverting the fix fails the test). Leg B #4966 → PR#4979 APPROVE-w-notes (exported `StripIssuePrefix`; `(#N)` train suffix structurally safe). Leg C #4967 → PR#4980 APPROVE-w-notes (consolidation complete + actually called everywhere; telegram's 7 extra v1 markers silently dropped — net improvement, noted on the PR). All three lkshrk-review defects are now genuinely closed.
**Created**: 2026-08-17
