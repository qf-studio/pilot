...
✅ **CUTOVER DONE 2026-09-05 (by Nelya) — `pilot.quantflow.studio` served from AWS since 09-05 19:21 CEST** (ECS behind ALB, Route 53; verified 09-06: DNS → AWS, HSTS + nosniff live, sub-pages 200). Handover received 09-06 in Slack `#infrastructure` (msg `1788692051.363079`, artifact "QuantFlow AWS Handover"). Her deploy leg: `qf-studio/pilot-docs` workflow "Deploy QuantFlow AWS" (`workflow_run` after our "Build image" on a `prod-*` tag → mirror GHCR → ECR → ECS). She also removed the GitLab pipeline files from pilot-docs and the nested `docker/docker/…` copies. **Step 1 (this repo, 09-05)** stands. **⚠️ Step 2′ URGENT → TASK-494:** our sync still wipes the target tree except `docker/` — the next `docs/**` merge deletes her deploy workflow and restores the GitLab files. No `docs/` PR is open as of 09-06; hold any until TASK-494 merges. Remaining: step 4 (retire GitLab, operator) + infra notes to Nelya (compression + CSP-RO missing on the AWS path; `PILOT_DOCS_PAT` = founder OAuth token, org admin → tag-ruleset bypass OK).
## Why
...
| 3 | Cutover: DNS → AWS ✅ 09-05 (Nelya) · headers verified 09-06 (HSTS, nosniff, XFO; **gzip + CSP-RO absent → raised with Nelya**) · sync GitHub-only + preserve target `.github/` + delete the two GitLab files + `/getting-started` 404 → **TASK-494** (Pilot) · `GITLAB_SSH_KEY` secret removal → operator after TASK-494 merges. | 🚧 TASK-494 |
