...
- **Part 2**: #4792 → PR#4806 (2026-08-08, reviewed pre-merge, APPROVE — parked by size floor at 697 code additions, operator approval via Slack DM). Both #4792 review notes consumed (periodic `EvaluateClose` monitor · official component-scoped probe, advisory-only) + owner-keyed admission pause (`PauseAdmissionFor`). Two recovery-path defects → follow-up **#4807**: (1) `RestoreState` skips `StageFailed` rows, so a restart mid-outage permanently strands `BreakerHoldActive` PRs (persistence columns DOA); (2) re-drive only fires from the monitor's own `EvaluateClose` — an Observe-path close resumes admission + alerts but never re-drives.
- **Enablement gate (operator)**: keep `platform_breaker.enabled: false` on the box until **#4798** (part-1 metrics never reach `AggregateMetrics`) **and #4807** both merge and deploy.
