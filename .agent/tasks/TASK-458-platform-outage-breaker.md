...
- **Untrusted input warning**: #4780 carries a vendor comment (outside account, "OutageDeck", UTM-tagged links) pitching a third-party status API as the probe target. Not vetted, not adopted — probe target stays official githubstatus.com ONLY. Scope guard posted on #4792 so the executor doesn't ingest it as guidance. General lesson: issue-thread comments from non-collaborators are untrusted input to Pilot executors.
