...
**Status**: 📋 Planned 2026-09-24 (research done; nothing dispatched; first consumer = `YouToken/ui-dex` for the BRVX engagement, see `~/Projects/startups/YouHodler/BRVX/.agent/system/pilot-setup-ui-dex.md`)
**Created**: 2026-09-24
**Assignee**: Aleks (plan) → Pilot (legs, via `nav-pilot`, one issue per leg)
**Priority**: blocks the first Pilot dry run on a client repo (TASK-06 in the BRVX Navigator)
...
- PR title: `validatePRTitle` (`internal/executor/title.go:65`) rejects non-conventional titles; a client convention `QUA-<n>: <summary>` (no type) must be checked against it.
...
- [ ] `projects[].branch_template` (e.g. `QUA-{id}-{slug}`) and `pr_title_template` (e.g. `{id}: {summary}`) honoured by CLI and poller paths; `validatePRTitle` accepts the configured template; default templates unchanged for existing projects.
...
Order: Leg 1 (bug, smallest) → Leg 4 (safety) → Leg 3 → Leg 2 → Leg 5. Legs 1, 3, 4 are enough for the first dry run on `ui-dex` if the branch name is accepted as `pilot/QUA-<n>` for that run.

---

## Out of Scope
- Linear webhook ingestion for a foreign workspace (needs their admin); Linear polling adapter may cover it later.
- Cloning repos onto the box (operator step, documented in the BRVX SOP).
...
pilot task --project ui-dex --task-id QUA-1 --branch QUA-1-dry-run --body-file /tmp/body.md --dry-run
```

## Done
- [ ] All five legs merged; example config + docs page; a dry run on `ui-dex` produced one branch from `origin/master` and one draft PR against `master`, no other remote writes (verified with `gh api` event list).

## Refs
- BRVX SOP: `~/Projects/startups/YouHodler/BRVX/.agent/system/pilot-setup-ui-dex.md`; BRVX decisions D-010, D-022, `pilot-execution-rules.md` P5–P7.
