...
**Status**: 🟢 **L2 DONE 2026-09-23 11:12Z — `ami-01ecd7cefa1e6f316` (pilot 2.276.1, CC 2.1.220, gh 2.63.2), run 35851941616 green; both SSM params updated. L3 step 1 DONE 13:07Z: reconciler on the new AMI (run 35863755066, task def rev 5, `tenant_factory=false`). Factory ON (rev 6, 13:19Z). **ui#170 fixed 09-24; first real provision 14:21Z failed on reconciler IAM (Nelya fixed 14:42Z); Deprovision → Provision is the next click (TASK-495 log).** History: L1 MERGED 2026-09-23 (infra#10 → PR#11, squash `118182ef`, review APPROVE-w-notes → follow-up [infra#12](https://github.com/qf-studio/aws-infrastructure-pilot/issues/12)); **L2 bake attempt 1 (run 35849443228) FAILED at staging: `mgmt-infra-admin` has no `s3:PutObject` on `pilot-s3-agent-data/binaries/pilot/*`** (download + sha256 OK). **Not IAM — the bucket policy requires SSE-KMS**; Nelya fixed the staging line on main (`7171c35`, `--sse aws:kms --sse-kms-key-id alias/pilot`). **Bake attempt 2 = run 35851941616** (from main). infra#12 → PR#13 merged `b4f6df9d` (staging after the guard, SSE kept, hard step per Nelya; manual rebase again). Gate to the first customer after login passed (TASK-495).
**Created**: 2026-09-22
...
- **09-23 13:07Z** Nelya picked (a): rebalancing DISABLED on console-reconciler, template on S3 since 13:15Z (answered 12:54Z after a reminder). Re-run 35863755066 (`prod-0.1.0`, `tenant_factory=false`) **green**: reconciler stack `UPDATE_COMPLETE`, service steady 1/1 on task def rev 5; verified `PILOT_CONSOLE_FLEET_AMI_ID=ami-01ecd7cefa1e6f316`, `PILOT_CONSOLE_FLEET_TENANT_FACTORY=false`, image `prod-0.1.0`. Posted in the AMI thread. Founder go → run 35865704231 (`tenant_factory=true`) green 13:19Z, task def rev 6 verified (`TENANT_FACTORY=true`, new AMI). **First tenant NOT provisioned: console.pilotcloud.dev is the mock UI build** (see TASK-495 09-24 entry; [ui#170](https://github.com/qf-studio/pilot-console-ui/issues/170)). Founder's connect/key/Provision hit fixtures; reconciler saw no row. ui#170 shipped 09-24 07:xxZ (PR#171, deploy 35966679422, live 961bd42 is the HTTP build). L3 resumes now: real register → connect GitHub → key → Provision → EC2 from `ami-01ecd7cefa1e6f316` → Docs real tree. Old AMI stays registered.
