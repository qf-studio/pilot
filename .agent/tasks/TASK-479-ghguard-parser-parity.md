...
**Status**: ✅ **COMPLETE — both legs merged + post-merge reviewed.** Parser [pilot#4963](https://github.com/qf-studio/pilot/issues/4963) → PR#4975 merged 08-18, reviewed APPROVE-w-notes (spec's G5–G7 `-p` assumption was wrong; implementation corrected it). D5 env bypass [pilot#4968](https://github.com/qf-studio/pilot/issues/4968) → PR#4981 merged 08-18, reviewed APPROVE-w-notes 08-19 (precedence matches gh's `-R > GH_REPO > remote`; host-check-before-repo-check ordering verified load-bearing; no fail-open path). **Residuals filed as [pilot#4986](https://github.com/qf-studio/pilot/issues/4986)** (D2 graphql+fields deny + the `--hostname` argv twin of GH_HOST, policy.go:280 actionNone). #4905 (lkshrk) can land GET+fields on top of the hardened parser whenever the contributor returns. Archive when #4986 merges.
**Created**: 2026-08-17
