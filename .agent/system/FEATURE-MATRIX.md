...
| gh-guard shim | ✅ | executor/ghguard | `pilot gh-guard` (internal, shim-invoked) | `claude_code.gh_guard` (default true) | Preventive counterpart to the GH-4670 sibling-issue-mutation audit: PATH-shims `gh` inside the Claude Code subprocess so every `gh` call is classified against the task's own issue/PR/branch before it runs, not just detected after the fact. Pure-function policy table (`ghguard.Classify`) allows read-only commands unconditionally, allows own-artifact mutations (`pr create` on the task branch, `issue`/`pr comment` on the task's own issue/PR) only when they target the task's issue/repo/branch, and denies issue/PR lifecycle mutations, cross-repo (`-R`/`--repo`) targets, non-GET `gh api`, and release/repo/secret/variable/workflow mutations by default. Fails closed for mutations and open for reads when task context is incomplete; fails hard (exit 126) if the real `gh` binary can't be resolved. Denied calls are journaled to disk by the shim subprocess and ingested post-run into the same `executor.github_sideeffect` event stage and `AlertEventTypeGithubSideEffect` alert channel GH-4670 uses (GH-4671, containment cluster for the GH-4649 incident) |
