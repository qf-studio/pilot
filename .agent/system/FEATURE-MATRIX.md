...
| Gateway bearer auth wired in production | ✅ | gateway, pilot, cmd/pilot | - | `auth.token` (`PILOT_GATEWAY_TOKEN` env) | `Authenticator.Middleware` existed (GH-4748/PR#4752's composed test asserted 401s) but both production construction sites — gateway mode (`internal/pilot/pilot.go`) and polling mode (`cmd/pilot/main.go`) — called `gateway.NewServer` (nil auth) directly, so the top-level `Config.Auth` field (already validated in `Config.Validate()`, already rendered by the pilot-console proxy's tenant config as `auth: {type: api-token, token: "${PILOT_GATEWAY_TOKEN}"}`) was silently ignored — any peer reaching a non-loopback gateway could approve PR merges. New `gateway.NewServerFromConfig(gatewayCfg, cfg.Auth)` is the single decision point both sites now call: non-empty token → `NewServerWithAuth` (auth enforced); empty/nil token → `NewServer` (today's behavior, unchanged, relies on the default 127.0.0.1 bind). No new config field, no console-repo change needed — the console already sends the token (GH-4756) |
