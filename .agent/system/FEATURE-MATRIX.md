...
**Last Updated:** 2026-09-24 (GH-5217)
...
| Acceptance-evidence gate (command allowlist + redaction + confinement + timeout) | ✅ | executor | - | `executor.acceptance_evidence` | GH-5437, part 1 of the PR #5436 security revision (part 2, classifier/wiring-pin work, is a separate follow-up issue). Classifies issue acceptance-checklist items as paste-output or mutation-style (`acceptance_evidence.go`, carried over from #5436 unchanged — no defect there) and runs them post-implementation in the task's worktree, appending `## Evidence`/`## Not verified` to the PR body via `Runner.appendAcceptanceEvidence` (5 call sites in `runner.go`, one per PR/MR-creation path). Three defects #5436 shipped are fixed here: (1) commands are checked against a first-token allowlist (`go`/`make`/`npm`/`npx`/`bun`/`bunx`/`pnpm`/`yarn`/`pytest`/`cargo`, configurable via `EffectiveAllowedCommands`) *before any subprocess spawns* — a disallowed command in a multi-command item blocks the whole item with zero calls, reported "Not verified: command not in allowlist"; allowed commands run in `sanitizedAcceptanceEnv()`, a fresh env containing only `PATH/HOME/TMPDIR/LANG/GOPATH/GOCACHE/GOFLAGS/GOMODCACHE/NODE_ENV/CI`, never the executor's own process env, and captured output is redacted (`acceptance_evidence_redact.go`: parent env-var values whose *name* matches `(?i)token\|key\|secret\|password\|credential`, plus repo secret-shape regexes) before being pasted into the (public) PR body. (2) the deterministic "delete line N" mutation resolves its issue-supplied path through `resolveWorktreeConfinedPath` (`acceptance_evidence_confine.go`: `filepath.Clean` + reject absolute + `filepath.EvalSymlinks` on both worktree root and candidate + require `filepath.Rel` not to start with `..`), refusing relative-`..`, absolute-path, and symlink-escape vectors with "Not verified: path escapes worktree" and no write; a symlink pointing to another file *inside* the worktree is still allowed. (3) every command runs under `context.WithTimeout` (default 10m, `EffectiveCommandTimeout`) via `shellAcceptanceCommandRunner`, reusing the existing `configureProcessGroup`/`killProcessGroup` + `cmd.Cancel` idiom from `parallel.go` to SIGKILL the whole process group on expiry, reported "Not verified: timed out after `<d>`". Secret-shape regexes live in one place (`internal/executor/secretpatterns`, `//go:embed patterns.txt`) consumed by both this redaction path and `scripts/check-secret-patterns.sh`, so the two never drift. `enabled: false` (or a nil-safe config, `IsEnabled`/`EffectiveAllowedCommands`/`EffectiveCommandTimeout` all handle a nil `*AcceptanceEvidenceConfig`) makes the PR body byte-identical to pre-#5437 behavior |
