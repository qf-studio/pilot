...
**Last Updated:** 2026-08-01 (v2.151.0)
...
| Self-upgrade admission pause | ✅ | executor + upgrade + main | - | - | GH-4683: on a saturated queue, self-upgrade's drain used to wait for ALL active executions (running + queued) while the dispatcher kept admitting new work mid-drain — a busy box could never empty its queue and every attempt timed out (2 windows, 30 min each, incident 2026-08-03). `Dispatcher.{PauseAdmission,ResumeAdmission,IsAdmissionPaused}` now stop `processQueue` from picking up new queued tasks (via a shared `admissionPaused *atomic.Bool`) without touching `QueueTask`/insertion, so pollers/retries can keep enqueueing — queued rows just wait. `Monitor.{GetActiveRunningTaskIDs,WaitForRunningTasks}` add a running-only sibling to the pre-existing running-or-queued `GetRunningTaskIDs`/`WaitForTasks` (left untouched — `autopilot.controller`'s orphan-running sweep relies on the broader semantics), wired into the hot-upgrade path via a new `runningOnlyTaskChecker` adapter in `cmd/pilot/upgrade.go`. `cmd/pilot/main.go` pauses admission before calling `PerformHotUpgrade` and resumes on every exit path except a successful Unix exec-restart (process is replaced) — Windows success still resumes explicitly since the old process keeps running there. Regression-pinned by `TestDispatcher_PauseAdmission_BlocksNewPickupButNotRunningTask`, `TestMonitorGetActiveRunningTaskIDs`, `TestMonitorWaitForRunningTasks_IgnoresQueuedTasks`, `TestMonitorWaitForRunningTasksTimeout` (v2.252.x) |
