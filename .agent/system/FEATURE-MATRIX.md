...
**Last Updated:** 2026-08-06 (v2.151.0)
...
| Production bearer auth on /api/v1 | ✅ | gateway/pilot | - | top-level `auth:` (`type`, `token`, expands `${PILOT_GATEWAY_TOKEN}`) | `Authenticator.Middleware` (auth.go) always existed and PR#4752's composed tests exercised it, but neither daemon entrypoint ever built the server with it — both `internal/pilot.New` (gateway mode) and `cmd/pilot/main.go` `start` (polling mode) called `gateway.NewServer` (nil auth) unconditionally, so a non-loopback bind (hosted instances, `host: 0.0.0.0`) served `/api/v1/` — including the approval-decision route — to any network peer. `gateway.NewServerFromConfig(cfg.Gateway, cfg.Auth)` is now the single decision point both call: auth enables only when `cfg.Auth.Token != ""`, otherwise identical to `NewServer` (backwards compatible, relies on the default `127.0.0.1` bind). pilot-console's `RenderTenantConfig` already emits `auth.type: api-token` + `token: "${PILOT_GATEWAY_TOKEN}"` for hosted tenants (`configrender.go:53-55`) and the console proxy already sends the matching bearer (`proxy.go:336`) — this closes the daemon-side half of that path; no console-repo change needed (GH-4756, PR#4752 review) |
