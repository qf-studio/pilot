...
| Gateway bearer auth enforced in production | ✅ | gateway/config | - | `auth.token` (`${PILOT_GATEWAY_TOKEN}` in hosted tenant configs) | `Authenticator.Middleware` and `gateway.NewServerWithAuth` existed since PR#4752 but neither production construction site (gateway mode `internal/pilot.New`, polling mode `cmd/pilot runPollingMode`) called it — both used the auth-less `gateway.NewServer`, so `/api/v1/` (including the pre-merge approval decision route, GH-4748) served unauthenticated regardless of config. `Config.GatewayAuthConfig()` bridges `cfg.Auth.Token` into `gateway.AuthConfig{Type: AuthTypeAPIToken}` (forced regardless of configured `Type`, since `AuthTypeClaudeCode` ignores `Token` entirely) at both sites; empty/unset token → `nil` → identical to `NewServer` (backwards compatible, matches the `127.0.0.1` default bind). The hosted console (`pilot-console internal/fleet/configrender.go`) already renders `auth.type: api-token` + `auth.token: ${PILOT_GATEWAY_TOKEN}` into every tenant config alongside `gateway.host: 0.0.0.0` — no console-side follow-up required (GH-4756) |
