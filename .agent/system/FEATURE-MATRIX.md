...
**Last Updated:** 2026-09-12 (GH-5435)
...
| Acceptance-checklist evidence gate | ✅ | executor | - | `executor.acceptance_evidence.enabled` (default true) | GH-5435: six consecutive Pilot PRs shipped with a "paste the output into the PR body" / mutation acceptance item left unchecked and unpasted, requiring manual re-verification. `internal/executor/acceptance_evidence.go` classifies each `Task.AcceptanceCriteria` entry as paste-output (phrases "pasted into the PR body"/"paste the output"/"terminal output", extracting inline `` `command` `` spans), mutation (an edit cue — change/delete/remove — followed by `->`/`→` and a "... fails" assertion, extracting the change description and target test/package), or other (ignored). `acceptance_evidence_run.go` runs paste-output commands in the task's worktree via the injected `AcceptanceCommandRunner` and captures trimmed (≤60 lines) combined stdout+stderr; mutation items matching the deterministic "delete/remove line N in `<file>`" shape are applied to the named file, the target test/package is run, failing test names are recorded (a mutation that fails no test is reported, not omitted), and the file is reverted byte-for-byte — freeform mutation text that doesn't match this shape is reported under `## Not verified` instead of guessed at. `Runner.appendAcceptanceEvidence` (called from all 5 PR-body assembly sites in `runner.go`) appends `## Evidence`/`## Not verified` sections to the PR body; a byte-identical no-op when `acceptance_evidence.enabled: false` or the task has no acceptance criteria |
