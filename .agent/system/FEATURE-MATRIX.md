...
**Last Updated:** 2026-08-03 (v2.151.0)
...
| GitHub session-scope directive + post-run side-effect audit | v2.252.x | executor | `githubScopeDirective` appends a parameterized negative to the PILOT EXECUTION MODE prompt block naming the dispatched issue/repo/branch (never close/reopen/edit/label any issue, never mutate other issues/PRs/repos, never push off-branch); `auditGithubSideEffects` runs post-run (one `gh search issues` call, fail-open) to detect any OTHER issue in the task's repo closed/reopened during the run window, journaling `executor.github_sideeffect` + firing the `github_sideeffect` alert rule — detective only, no auto-revert (GH-4670, containment for the GH-4649 sibling-issue-close incident) |
