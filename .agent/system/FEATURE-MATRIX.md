...
| Opt-in release tagging for human merges | v2.223.x | autopilot | `release.tag_human_merges` (default false); `ScanRecentlyMergedPRs` also tracks non-`pilot/*` PRs merged to the default branch when enabled, gating Pilot-only merge metrics/self-heal/board write-back on `isPilotPR` (GH-3928) |
