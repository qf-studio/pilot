...
**Last Updated:** 2026-08-05 (v2.151.0)
...
| GitHub App installation-token auth | ✅ | adapters/github, cmd/pilot, executor | - | `adapters.github.app` | Opt-in `{app_id, installation_id, private_key_path}` block, validated eagerly at `Config.Validate()` (partial block errors naming the missing field). `github.TokenSource` (`apptoken.go`) mints an installation token via a hand-rolled RS256 JWT (no new go.mod dep) → `POST /app/installations/{id}/access_tokens`, caches it, and refreshes proactively 5min before the ~1h expiry. Sits ahead of the config-token/`GITHUB_TOKEN`/gh-CLI chain in `resolveGitHubToken`; on mint failure logs loudly and falls through to that chain rather than failing the caller. The same minted token authenticates pilot-worktree git push/fetch via a `GIT_ASKPASS` helper (`internal/executor/git_credentials.go`) that only ever passes the token through a child process's `PILOT_GIT_TOKEN` env var — never argv or a log line — installed once at daemon startup via `executor.SetGitCredentialProvider`. Kills the single-OAuth-grant SPOF and moves the daemon off the shared per-user 5000/hr rate pool onto a per-installation one (see `.agent/sops/config/github-token-architecture.md`). Known scope boundary left for follow-up: `gh` CLI subprocess calls (PR creation, issue comments) still ride ambient `GITHUB_TOKEN`/gh-CLI login, not this token (GH-4746). The long-lived in-tree client hot-rotation gap is fixed — see next row (GH-4747) |
| GitHub SDK client per-request token resolution | ✅ | adapters/github, cmd/pilot | - | - | `github.Client` (`internal/adapters/github/client.go`) takes a `TokenFunc` (`func(ctx) (string, error)`) instead of a fixed string; every REST/GraphQL/job-logs request resolves the token fresh via `resolveToken`, so a long-lived client survives a GitHub App installation-token refresh instead of replaying the boot-time token into 401s ~1h in (GH-4747, closes App-auth scope boundary (2) from GH-4743/`sops/config/github-token-architecture.md`). `NewClient`/`NewClientWithBaseURL` wrap a static string in `StaticToken` — zero behavior change when App auth is inactive. `cmd/pilot`'s `newGitHubClient(cfg)` (`main.go`) is the one chokepoint that bridges `resolveGitHubToken` into a `TokenFunc` via `NewClientWithTokenFunc`; wired into the three daemon-lifetime in-tree clients that actually needed it — the `/ready` GitHub verifier (`adapter_preflight.go`), the approval-handler + autopilot step-log `ghClient` (both autopilot-controller construction sites in `main.go`), and the comms `/draft-issue` issue creator. Deliberately NOT wired into every `github.NewClient` call site — one-shot CLI commands and startup-only validation calls have no rotation to miss, so converting them would be pure churn. Known remaining gap (out of scope, separate repo): the studio-sdk-based `githubSDK.Client` (`github.com/qf-studio/studio-sdk`) used by the SDK poller and autopilot's `Controller` still takes a static token string with no source hook — that client type lives in a different Go module/repo and can't be changed from a pilot-repo PR; needs its own ticket against studio-sdk |
