...
**Last Updated:** 2026-08-07 (v2.151.0)
...
| Per-project approval override | ✅ | autopilot + config + health | - | `projects[].approval.require_approval`, `projects[].approval.approval_source` | `ProjectApprovalOverride{RequireApproval *bool; ApprovalSource *ApprovalSource}` overlays the resolved env/global `RequireApproval` gate and `ApprovalSource` channel per project (mirrors the GH-4478 `ProjectCIChecksOverride` pattern) — unset fields inherit. `WithApprovalOverride` resolves it once in `NewController` into cached `resolvedRequireApproval`/`resolvedApprovalSource` fields (not re-derived per read, since — unlike CIChecks — these are read on every tick via `handleCIPassed`/`submitAsyncApprovalRequest`); wired at all three controller construction sites in `cmd/pilot/main.go` (gateway mode, default repo, projects loop). Lets one operator route a personal project's approvals to Telegram and a work project's to Slack, or gate one project more strictly than another, from a single shared `autopilot.Config`. `config.Validate()` checks `approval_source` against the same `telegram`/`slack`/`github-review` alias set `internal/approval/channel.go` recognizes; `pilot doctor` gained a per-project deadlock variant of the existing `approval-misconfig` check plus a new `approval-source-unregistered` check that flags a resolved `approval_source` (global/env/project) with no adapter+approval-block registered to serve it (GH-4774) |
