...
- **Fix-issue evidence quality** (observed 2026-08-09, PR#4818 lint cycle): the auto-generated `CreateFailureIssue` body for #4820 truncated the CI logs *before* the actual errcheck line — which WAS present in the run log (`gh run view --log-failed` surfaces it in one grep). The preflight judge caught it (`reject_vague`) — defense-in-depth worked — but the spawned-issue evidence should include the specific failing lines, not a truncated excerpt. Related TASK-459 theme: the issue body is the fix executor's evidence.
- **Dual-armed recovery race** (same cycle): after the CI-failure close of PR#4818, BOTH the fix-issue chain (#4820) and the source-issue retry chain (#4817 → PR#4821) were armed simultaneously. GH-3806's `TerminalLabel` mechanism exists to mark a follow-up issue as retry owner but didn't fire here. Benign this time only because preflight rejected #4820; two executors racing on the same failure is the double-dispatch shape GH-3806 was built against.
