...
**Current Version:** box runs **`Pilot v2.251.0`** — reached via the **first fully autonomous self-upgrade** (07-31 train cut 14:11Z; daemon downloaded → backed up → hot-restarted via exec → verified 14:13Z, zero operator; note: exec preserves ps start time, so board "uptime" spans upgrades). Operator graceful restart ~15:29Z same day activated per-project `ci_checks` overrides (backup `config.yaml.bak-2026-07-31-cichecks`). **Daemon runs on AWS** (`i-0e0c1ca34e7b561f9`, TASK-409; ops via `pilot-aws` skill; NO local daemon; binary `/var/lib/pilot/bin/pilot`, `/usr/local/bin/pilot` symlinks to it, rollback at `pilot.prev`) — **approvals OFF since 07-20** (auto-merge on green CI) — **GH-4391 rate-budget client LIVE** | full status in `.agent/system/FEATURE-MATRIX.md`
...
**Recent (July 26–31 2026; detail lives in `system/saas-roadmap.md` v8.8–v9.6, `tasks/archive/`, and git log — do not re-grow this block, replace it):**
- **07-31: first autonomous self-upgrade (v2.251.0) · drift-defects #6–#8 shipped+SOP-verified same day · release trains unblocked.** #6 ui#36 silent credential-save failure (PR#37) · #7 console#83 env-gated postgres secrets driver — the local stack now exercises the FULL credentials write path incl. live Anthropic key validation (PR#84; driver e2e 9/9) · #8 ui#38 error-envelope mismatch (PR#39, shipped 6 min after filing). Release-train incident RCA'd (TASK-431): global `required_checks: [test, lint]` named checks that never post on auth-service/studio-sdk → permanent CIPending → 18 scopes stuck/parked, auth 11 days unreleased; ops fix (per-project `ci_checks.required_checks: []` pointer idiom + superseded scopes → done + restart) released **auth v0.68.0 + sdk v0.31.2** within 2 ticks; durable fix **#4646→PR#4647 merged** (terminal `CIConfigMismatch`, honest park message, startup lint, stale-panel reconcile) — live on box at the Mon 08-03 train. Roadmap **v9.6**.
- **07-30: v2.250.0 manual cutover · spec-guard epic #4624 shipped (strike-loop class dead) · real-stack-verify SOP adopted (founder nod)** — `sops/quality/real-stack-verify-gates-ui-merges.md`; drift-class scoreboard hit 8-for-8 caught by operator real-stack verify, zero by fixture-only daemon gates · #4643/#4644 carrier-timeout park fix · takeover-race #4619/#4620 dispatched · #4265 closed (TASK-407 proof met). Roadmap **v9.5**, markers archived.
...
- **07-27/28: S2 EXIT MET · graph-drift prevention program shipped · epic-failure cluster killed (4 same-day fixes) · GH-4595 park stack · v2.247.0 self-shipped.** Roadmap **v9.0**, TASK-423/424/425/426 archived.
- **07-26: SaaS UNPARKED · hosted merge leg root-caused+fixed (autopilot never enabled on tenant — top-level YAML binds to nothing; first canary merges ever) · v2.246.1.** Pitfall mem-160.
- **Earlier July (compressed):** approvals-off 07-20 · rate-budget client #4495 · v2.244.0 zero-touch release · TASK-407 atomic-claim reliability track shipped+proven · S6-lite AWS cutover 07-16 (TASK-409) · #4393 split-brain recovery. Detail: git log + `tasks/archive/`.
