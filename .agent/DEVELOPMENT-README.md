...
**Current Version:** box build `v2.244.0-4-g0193916e` (deployed 07-21 17:42Z from main; release v2.244.0 cut by train 07-21 14:23Z — first fully autonomous train through a rate-limit failure, #4485's retry saved its own release; ⚠ first run of the `-4` build died silently ~2min post-boot with zero forensics, second run stable — rollback `/var/lib/pilot/bin/pilot.prev` if it recurs) — **daemon runs on AWS** (`i-0e0c1ca34e7b561f9`, TASK-409; ops via `pilot-aws` skill; NO local daemon; binary at `/var/lib/pilot/bin/pilot`, `/usr/local/bin/pilot` is a symlink) — **⚡ approvals OFF since 07-20 21:41Z**: auto-merge on green CI, proven unattended 07-21 (pointer #144/#145, pilot #4497) — **GH-4391 rate-budget client LIVE** (15% poller floor, ETag conditional scans, 72h/cursor/3s-stagger startup; metric `pilot_rate_limit_floor_engaged_total`) | full status in `.agent/system/FEATURE-MATRIX.md`
...
- **07-18 loop-class kill + big ship day:** the three destructive-close rungs in `autopilot/controller.go` (conflict `closeAndReexecute`, CI-fail always-close, size-guard self-close — destroyed #4402/#4419/#77 and the GH-4415 chain) replaced by **escalate-and-hold** via GH-4422 epic (#4461/#4462/#4463). Also landed: repick hard-cap counts restart churn/operator cancels (#4455), RLIMIT_AS→cgroup OOM cap (#4456), ledger completion-recording (#4457), terminal-status writes (#4464). **Slack Socket Mode approvals PROVEN end-to-end** (#4434, clicks → auto-merge, 6 PRs merged by button today). **Pointer waves 2–4 complete: TASK-12→19 all merged (#65–#78)**, #80 (TASK-18 re-run) in waiting_ci. Box upgraded `-18`→`-27` 13:29Z; on restart: TUI subtask ghosts cleared, zombie canary PR states healed to waiting_ci, false-stalled GH-4391 auto-re-dispatched (new retry path, no surgery). New pitfalls: `hard-cap-rearm-in-memory-gate` (4-step re-arm + in-memory gate), `alert-engine-counts-operator-cancels` (false CRITICAL on surgery days), `tui-subtask-ghost-rows`. **Unblocked next:** re-label GH-4415 & GH-4393 (parked victims of the dead loop class), GH-4387 re-dispatch (old work in `refs/pull/4419/head`). Standing: secret rotation (box gh token needs `read:project`), alert-engine cancelled≠failed issue to file.
...
- **Latest handoff marker**: `.agent/.context-markers/2026-07-20_backups-live-loop-fix-running.md`
