...
| Priority | Topic | Why |
|----------|-------|-----|
| **P0** | **`IntentCommand` unrouted + unsafe `default:→task`** ([TASK-372](tasks/TASK-372-command-routing-safe-default.md), 🚀 [#3657](https://github.com/qf-studio/pilot/issues/3657)) | `@Pilot /help` on Slack created a *task* (live 2026-06-25): comms dispatch has no `IntentCommand` case → `default: treat as task`; Slack never intercepts `/`-commands like Telegram does. Fix = route `IntentCommand` via shared `comms.CommandHandler` + make default a safe clarify (no auto-task). Same `default:→task` foot-gun behind the v2.193.0 operational regression. Dispatched. |
