...
**Current Version:** v2.186.6 | full status in `.agent/system/FEATURE-MATRIX.md`

**Recent (v2.186.0–v2.186.6, June 10–12 2026):** **Decomposition integrity wave 2 + release-stage hardening + executor SHA-harvest fix + safeGo sweep + retry-machinery hardening + TASK-364 holes 4/5 — all SHIPPED.**
- **v2.186.4–v2.186.6 — TASK-364 holes closed by Pilot itself (June 11–12):** hole 4 sanitizer (`sanitizeSubtaskDescription`) via the #3582 epic (#3590/#3591, regex tightened in #3595 after conflict self-heal + Telegram approval); hole 5 (recovered-children rehydration + empty-description refusal) via #3593 — a clean single-PR run under `no-decompose` that **live-verified TASK-320/355** (execution row: `completed` + real worker SHA matching the PR — #3224 closed, both tasks archived) and the wave-2 epic checklist (**held**: no premature close, no false supersession, conflict self-heal worked). The #3582 run also exposed the next decomposer bug — splits explicitly-standalone single-package tasks ([#3597](https://github.com/qf-studio/pilot/issues/3597), MANUAL): prose is never machine-checked, context-cited paths defeat `isSinglePackageScope`. Use the `no-decompose` label / `<!-- pilot:no-decompose -->` marker on standalone issues until it lands.
...
| P1 | **Decomposition integrity residue** | Waves 1+2 shipped (v2.183.0/v2.186.0); [TASK-364](tasks/TASK-364-decomposition-integrity-residual-holes.md) holes 4+5 shipped v2.186.4–6 (Pilot-built); wave-2 epic checklist **live-PASSED** on GH-3582 ([TASK-361](tasks/TASK-361-autopilot-decomposition-integrity.md)). Remaining MANUAL: [#3597](https://github.com/qf-studio/pilot/issues/3597) — decomposer splits explicitly-standalone single-package tasks (prose unchecked, context-cited paths defeat `isSinglePackageScope`). Workaround: `no-decompose` label on standalone issues. Teammate purge/PAT ops cancelled (mem-035). |
