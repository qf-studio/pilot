...
**Current Version:** v2.213.2 | full status in `.agent/system/FEATURE-MATRIX.md`
...
- **Defect burn-down — [TASK-382](tasks/TASK-382-restart-epic-defect-burndown.md):** 14/15 restart & epic-lifecycle defects from the shift shipped (register + issue/release links in-doc); D6 parked ([#3789](https://github.com/qf-studio/pilot/issues/3789), human-led `Blocked by:` enforcement — resolve blockers from the already-fetched candidates, no new API call). `auto_label_pilot` no-op ([#3710](https://github.com/qf-studio/pilot/issues/3710)) closed not-planned (vestigial field).
...
| Priority | Topic | Why |
|----------|-------|-----|
| ✅ | **Runtime self-verification** ([TASK-379](tasks/TASK-379-runtime-self-verification.md)) | **SHIPPED — all 8 waves (V1–V8), 2026-07-04.** doctor/`/ready` live auth probes, fail-loud degraded paths, execution ledger + `execution_events` + `pilot trace`, shared Anthropic builder, and a **synthetic E2E canary validated live** (issue→daemon→PR→merge). Remaining: canary cron `disabled_manually` pending an **auto-merge design call** (project pipelines vary — don't build naively); rotate `CANARY_GH_TOKEN` (user). |
| P0 | **Restart & epic-lifecycle defect burn-down** ([TASK-382](tasks/TASK-382-restart-epic-defect-burndown.md)) | 🚀 Dispatched. Defects from the July-3 autonomous shift; register with issue links + release versions in the task doc. One parked (#3789, human-led `Blocked by:` enforcement — resolve blockers from already-fetched candidates, no new API call). |
