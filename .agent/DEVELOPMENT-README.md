...
| **P1** | **CI-fix continuation drops original commits** ([#5348](https://github.com/qf-studio/pilot/issues/5348), dispatched 09-07) | **root-caused 09-07:** executor force-resets the fix branch to main by name (empty base + `-B`), recorded SHA never parsed, own close misread as external → superseded label + branch delete. Shared cause of the week's three incidents: prose continuation state · name-based git · labels on process events, not delivery evidence. Pitfall `ci-fix-continuation-rebuilds-branch-from-main-drops-original-commits`. |
