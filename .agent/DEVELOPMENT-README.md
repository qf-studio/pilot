...
- **Latest handoff marker**: `.agent/.context-markers/2026-09-21_console-live-on-pilot-fleet-domain-decided.md`
...
| **P1** | **Fleet TO-BE alignment (Nelya)** ([TASK-495](tasks/TASK-495-fleet-to-be-alignment-nelya.md)) | 🟢 **BOTH CONSOLE SERVICES LIVE 09-21** (run 35579391317; 10/10 alarms). Her side next: gRPC NLB :4002 + SES identity (login/email) · register `pilotcloud.dev` + phase 2 (ALB/ACM/CloudFront). Ours: MFA enrol (founder) · validation 1–3 · fresh golden AMI · port-forward SOP · terminate old tenant + retire CDK (~$420/mo) · EU/UK trademark check · close infra#51 as superseded. |
...
| P2 | **Console Docs page — live verification** ([TASK-466](tasks/TASK-466-console-docs-page.md)) | Read leg shipped 08-19, never rendered live (no in-VPC console until 09-21). Daemon leg proven 200 on both boxes; console proxy/UI correct. Now blocked only on: login (Nelya gRPC) + a tenant in `pilot-fleet` (AMI + factory on). Design's project picker can't work (daemon scopes one `SetDocsProjectPath`). TASK-498/infra#51 withdrawn — grants already exist in her TaskBoundary. |
