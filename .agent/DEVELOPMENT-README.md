...
**Current Version:** box process runs **`v2.259.1`** — the 08-12 14:18Z self-upgrade **hot restart took effect** (`upgrade verified complete … via "hot restart"` in daemon.log; PR#4861/4862 live since then; redundant operator bounce 08-13 10:32Z). ⚠️ **The "disk≠process mismatch" signature is RETIRED — it was the same misdiagnosis ×3** (memory `hot-restart-preserves-pid-uptime-false-mismatch`): the restart leg is `syscall.Exec`, the PID is preserved, so board `uptime` (ps etime) survives hot restarts and proves nothing; board `ver` reads the disk binary. Until [pilot#4864](https://github.com/qf-studio/pilot/issues/4864) ships a running-version surface (`pilot_build_info` metric · real /health version · doctor 3-way check), the truth source is the daemon.log boot-reconcile line. **`platform_breaker` ENABLED in production since that restart** (config: `orchestrator.autopilot.platform_breaker.enabled: true` — NOT top-level `autopilot:`, which binds to nothing; backup `config.yaml.bak-20260808-breaker`; armed-state startup log = #4814). **Daemon runs on AWS** (`i-0e0c1ca34e7b561f9`, TASK-409; ops via `pilot-aws` skill; NO local daemon; binary `/var/lib/pilot/bin/pilot`, rollback at `pilot.prev`) — **approvals OFF since 07-20** (auto-merge on green CI; size-floor/scope-drift escalations park `awaiting_approval`, asks route channel-first per #4810, live since the rebuild) — **GH-4391 rate-budget client LIVE** | full status in `.agent/system/FEATURE-MATRIX.md`
...
**Recent (Aug 4 – Aug 13 2026; detail lives in `system/saas-roadmap.md`, `system/approval-architecture-roadmap.md`, `tasks/archive/`, and git log — do not re-grow this block, replace it):**
