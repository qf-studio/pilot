...
**Current Version:** v2.238.0 | full status in `.agent/system/FEATURE-MATRIX.md`
...
| P3 (ops) | **Host cache reclaim + GitLab registry retention** ([#3380](https://github.com/qf-studio/pilot/issues/3380)) | **Human-led, NOT a `pilot` task** (needs infra access). One-time reclaim of stale `/data/quantflow/pilot_cache` + GitLab Container Registry cleanup policy; also Nextra v4.6.1 a11y upstream bugs. Follow-up to docs cache-bloat fix (#3287/#3373). Context: `.agent/system/docs-cache-and-lighthouse.md`. |
...
| P1 | **GH Projects board as work source** — Studio SDK roadmap | ✅ Read path (TASK-317) + full board-driven lifecycle loop (TASK-319, LIVE-verified) + daemon-loop hardening (TASK-356, v2.166.7–9) + board-orphan defense-in-depth ([TASK-354](tasks/archive/TASK-354-board-orphan-in-progress.md), v2.180.0) all **shipped & archived**. Remaining: **M7 SDK cutover** — 9/10 adapter poll/chat paths consume studio-sdk; **github is last + can only partially cut over** (Phase 4a poll-path scaffolding SHIPPED dormant via [#3638](https://github.com/qf-studio/pilot/pull/3638), **closed out on studio-sdk v0.25.0** via [#3711](https://github.com/qf-studio/pilot/pull/3711); 4b–4d **re-confirmed blocked at v0.25.0** — board layer, 5 client methods, poller-option surface all absent → gate **v0.26.0**, spec'd as [studio-sdk#71](https://github.com/qf-studio/studio-sdk/issues/71)). **4b + 4d.1/4d.3/4d.4 SHIPPED 2026-07-06 (PRs [#3890](https://github.com/qf-studio/pilot/pull/3890)/[#3894](https://github.com/qf-studio/pilot/pull/3894)/[#3895](https://github.com/qf-studio/pilot/pull/3895)/[#3896](https://github.com/qf-studio/pilot/pull/3896), studio-sdk v0.28.1): SDK poller live behind `use_sdk_poller` (default repo; flag set, daemon restart pending — user); autopilot fully on the SDK client (2 upstream parity releases forced: v0.28.0 typed errors + methods, v0.28.1 stale-`GetTagForSHA` fix — pitfall `pitfall_sdk_ports_go_stale_vs_intree.md`); spec-guard enforced on the SDK path; SDK PR-create via per-repo runner registry (gh CLI fallback). 4d.2 projects-loop + 4d.5 webhook repoint (#4154) **SHIPPED**. **4d.6 cleanup in progress** ([#4155](https://github.com/qf-studio/pilot/issues/4155), delete-vs-residual DECIDED — gitlab precedent kept a residual): board-source swapped to SDK ([#4168](https://github.com/qf-studio/pilot/issues/4168)→PR #4173) + `cmd/pilot/main.go` flag-off code pruned ([#4170](https://github.com/qf-studio/pilot/issues/4170)→PR #4175) done; remaining — delete dead in-tree poller machinery ([#4169](https://github.com/qf-studio/pilot/issues/4169)), deprecate `use_sdk_poller` with a warn-and-ignore ([#4171](https://github.com/qf-studio/pilot/issues/4171)), docs sweep ([#4172](https://github.com/qf-studio/pilot/issues/4172)). SDK gate history: [TASK-385](tasks/archive/TASK-385-studio-sdk-v0260-github-surface.md).** [TASK-368](tasks/TASK-368-m7-github-cutover-phase4.md) / [#3423](https://github.com/qf-studio/pilot/issues/3423). Board-loop tail [TASK-355](tasks/archive/TASK-355-board-sourced-noop-false-positive.md): ✅ shipped v2.186.2, live-verified + archived. |
