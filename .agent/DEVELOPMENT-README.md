...
**Recent (Aug 6 – Aug 17 2026; detail lives in `system/saas-roadmap.md`, `system/approval-architecture-roadmap.md`, `tasks/archive/`, and git log — do not re-grow this block, replace it):**
...
- **08-12 (incidents-first day): PR#4846 incident CLOSED + a 3-generation same-day fix cascade, all reviewed.** Founder rule recorded: **incidents are always first** (memory `incidents-always-first`). PR#4846 pre-merge-reviewed (HOLDS WITH DEFECTS, tests mutation-verified) → manually merged 21:50Z 08-11 (merge was the safe verb — close would have armed retry on shipped work; daemon finalized labels/branch but skipped its issue-close leg, #4841 closed manually). Root-cause + review defects dispatched and each generation's review spawned the next, all same-day merges: **#4851→PR#4853** (confirm-poll before CI deadline; APPROVE-with-follow-ups) + **#4852→PR#4854** (owner-death hardening; HOLDS) → **#4855→PR#4857** (re-entry clock reset; APPROVE) + **#4856→PR#4858** (review-claim guard; HOLDS) → **#4859→PR#4861** (mid-wait SHA-change clock reset) + **#4860→PR#4862** (seeded-claim test · dedup reuse-don't-replace · CI-path claim release) — **PR#4861/4862 post-merge reviews PENDING**. Box rebuilt+restarted twice (operator kill mid-day): process on v2.259.0-14; train cut **v2.259.1** (disk-only until next restart). 8 task docs archived (TASK-463/465/467/468/469/470/472/473).
- **08-11 (full day): design sprint ×4 pages + double review wave + C16/C17 SHIPPED END-TO-END.** Design (ui repo `design/`): board v1 tuned · **docs-v1** NEW (TASK-466) · **issue-v1** NEW · **connections-v1** draft in `/tmp` (founder iteration pending). Reviews wave 1 (PR#4837/4838/4839/4840): 4840 exposed 2 majors → founder "dispatch em all" → #4841–#4845 + sdk#111 + console#115 → 4 of 6 merged same day (PR#4848/4849/4850 + sdk PR#112) + C16 PR#116 + C17 console PR#117. Wave-2 verdicts posted on the PRs. #4845 still gated on the next sdk train (+re-anchor per TASK-471).
- **Earlier (compressed):** 08-06→08-08 GH-Actions outage → recovery → hardening wave (TASK-458 breaker enabled in prod; detail `system/approval-architecture-roadmap.md`) · 08-04/05 TASK-441 contract hardening + first unattended self-upgrade (v2.253.0) + S4 waves 2–4 + token incident resolved · 08-01/08-03 S4 wave 1 + Golden AMI v2 merged (operator bake pending) · 07-31 first autonomous train + AWS cost audit (`cdk deploy` pending) · 07-30 spec-guard epic + real-stack-verify SOP · 07-29 S3 backend 10/10 · 07-27/28 S2 EXIT MET · 07-26 SaaS UNPARKED · 07-20 approvals off · 07-16 S6-lite AWS cutover (TASK-409). Detail: git log + `tasks/archive/`.
...
| **P1** | **gh-guard parser parity** ([TASK-479](tasks/TASK-479-ghguard-parser-parity.md)) | **11 verified parity gaps on main** — our parser isn't pflag-faithful, so attached shorthand (`-XPOST`), attached data flags (`-fstate=closed`), bundles (`-pXDELETE`), context-insensitive `valueFlags` (`-p -X POST`, `pr create -f --head`), and attached `-R` cross-repo writes all classify as allowed reads while `gh` executes the mutation. Fix = per-command flag specs + pflag-faithful parse (attached/`=`/last-wins/`--`) + split `hasDataFlag` into fields vs `--input` (always a body). Spec has ~30 regression rows. D5 needs its own issue: `GH_REPO`/`GH_HOST` env bypasses the repo check identically and no argv fix covers it. Research complete, NOT dispatched. |
| **P1** | **Safe no-op / decline contract** ([TASK-480](tasks/TASK-480-safe-noop-decline-contract.md)) | Executor has no way to say "correctly nothing to change" — external PR #4901 tried inferring it from the *mandatory* exit-success signal, which masks the GH-916 forgot-to-commit class (~10% of failures), makes the no-commit retry unreachable, and bypasses GH-4517 preserve (deletes real work). Fix = dedicated `no_op:true`+mandatory-reason signal, `preserveDirtyWorktreeAsWIP` verified clean BEFORE any decline classification at all three insertion points. Also closes a pre-existing latent bug (post-retry DECLINED check runs before the preserve call today). Sibling of TASK-460. Research complete, NOT dispatched. |
| P2 | Review follow-up defects ([TASK-481](tasks/TASK-481-review-followup-defects.md)) | Three independent legs from the lkshrk review: **A** Linear `GetLabelByName` filters `team.key` only, so a UUID-configured `team_id` re-creates the label every startup (unfixed half of #4884, composes with merged #4896) · **B** `auto_merger` strips only literal `GH-<n>: `, so compound/Jira prefixes leak into squash subjects — `title.go:25` already claims parity it doesn't have; fix = exported `StripIssuePrefix` · **C** slack/discord `CleanInternalSignals` is **dead code never called by `FormatTaskResult`** — signal blocks leak to those users unconditionally today; consolidate all four copies onto `comms.CleanInternalSignals` (unblocked: #4903 merged). Research complete, NOT dispatched. |
