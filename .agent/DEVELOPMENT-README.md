...
| **P1** | **Executor false no-op after backend timeout** ([#5342](https://github.com/qf-studio/pilot/issues/5342), dispatched 09-06) | finalization reuses the expired task ctx; committed work reported `no_op`, never pushed (pilot-console GH-263 ×3). Pitfall `backend-timeout-finalization-on-expired-ctx-discards-committed-work`. |
