...
**Current Version:** v2.166.16 | full status in `.agent/system/FEATURE-MATRIX.md`
...
**Previous (v2.166.9, June 2 2026):** **TASK-356 daemon-loop fixes — CLOSED.** epic-decompose work-loss now fails loud + harvests `CommitSHA` only after the no-commits guard (#3383, `no-decompose` escape hatch, pitfall `mem-023`); externally-merged-PR board write-back via `ScanRecentlyMergedPRs`, decoupled from on_merge release (#3391/#3395). Plan: `.agent/tasks/archive/TASK-356-epic-decompose-workloss-and-loop-findings.md`.

**Earlier (v2.166.6, June 1 2026):** TASK-322 self-audit **Waves 0–3 complete** (3 crit · 14 high · 17 med). Only **Wave 4 (13 lows)** remains, gated on a ~2-week re-audit (**TASK-357**, gate ~June 15). `make release` is **tag-only** (#3377 — goreleaser sole publisher). Roadmap: `.agent/tasks/TASK-322-remediation-roadmap.md`.
