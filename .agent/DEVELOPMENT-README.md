...
**Recent (Aug 25 – Sep 4 2026; detail lives in `system/saas-roadmap.md`, `system/approval-architecture-roadmap.md`, `tasks/archive/`, and git log — do not re-grow this block, replace it):**
...
| **P1** | **Fleet TO-BE alignment (Nelya)** ([TASK-495](tasks/TASK-495-fleet-to-be-alignment-nelya.md)) | 09-06 founder decisions: **ECS console** · accept her CloudFormation over our CDK · founder box stays · her pipeline builds console image from `prod-*`. 9/11 of her code findings confirmed; fixes dispatched pilot-console #259–#264 + cloud-infra user-data fix. **Owed:** ECS env manifest after merges · template review · domain pick (still parked). |
| **P1** | **pilot-docs → GitHub + AWS deploy** ([TASK-492](tasks/TASK-492-pilot-docs-gitlab-to-github-migration.md)) | **CUTOVER DONE 09-05 by Nelya** — `pilot.quantflow.studio` on AWS (ECS+ALB), handover 09-06. **⚠️ URGENT [TASK-494](tasks/TASK-494-docs-sync-github-only-post-aws-cutover.md) → [#5315](https://github.com/qf-studio/pilot/issues/5315):** our sync still wipes pilot-docs except `docker/` — next `docs/**` merge deletes Nelya's deploy workflow + restores GitLab files. **Sync rewrite MERGED (PR#5320) + proven e2e 12:10 — hold lifted; deletions child #5317 failed CI (stale gitlab-ci check) → fix #5322 re-scoped in body; #5323 in CI.** auth-service asks filed: [#506](https://github.com/qf-studio/auth-service/issues/506)/[#507](https://github.com/qf-studio/auth-service/issues/507). Sibling: qf-website [#18](https://github.com/qf-studio/qf-website/issues/18) (headers/deploymentId/Vercel SOP). Remaining after: step 4 retire GitLab (operator: `GITLAB_SSH_KEY` secret, `gitlab:` config block, archive repo), auth-service dispatched 09-06: [#508](https://github.com/qf-studio/auth-service/issues/508) DPoP scheme behind ALB ("SSL not working") · #507 drop `TLS_ENABLED` · #506 per-app `aud`; operator step after #508 merges: set `TRUSTED_PROXY_CIDRS` SSM param (Nelya). |
