...
- **08-19 (afternoon): third wave — Pilot cleared the reachability chain same-day, all 5 reviewed, PR#5002 pre-merge APPROVED.** Merged+reviewed: #4997→**PR#5000** APPROVE-w-notes (spawn gate genuinely re-reads PR/issue state at spawn time; incident repro test; follow-up candidate: `spawnReviewIssue` shares the staleness class, ungated) · #4998→**PR#5001** APPROVE-w-notes (`Comment.Body`→ADFText, sole comment-bearing struct, webhook inherits) · sdk#123→**PR#125** APPROVE-w-notes (OnPRCreated now bridged by ALL 7 tracker adapters; sweep found **9 more github-only PollerDeps fields** → **sdk#127** filed, conformance-test approach recommended) · sdk#124→**PR#126** APPROVE-w-notes (statusCategory fallback verified to decode from default /transitions on Cloud+Server; Russian fixtures genuine) · #4999→**PR#5002 pre-merge APPROVE** (idempotency centralized in `notifyJiraDone` + immediate persist, both call sites, full store round-trip incl. repo-scoping rebuild, SQLite restart-replay test). **KAN-6 chain now code-complete but NOT active**: needs sdk tag past v0.35.1 (today's 14:00Z sdk train) → pilot pin bump → box rebuild past v2.262.0; historical KAN-6 card itself needs a manual move or a fresh KAN test card (untracked-era PRs never retro-process). Milestone: **#5000 = Pilot's own spawner fix, merged**. 🎉
- **08-19 (midday): Pilot executed the whole morning batch same-day; second review wave ×7 caught a FALSE DELIVERY.** Merged + reviewed: **#4985→PR#4991 APPROVE** (Linear `ID!`; reviewer mutation-tested the declaration pin — TASK-481 CLOSED+archived) · #4988→PR#4996 APPROVE-w-notes (judge diffs from fresh merge-base at both judge sites; regression test proves the old path bundled sibling work; gen-1 PR#4994's CI failure was fixture `init.defaultBranch` fragility, retry fixed it) · #4989→PR#4993 APPROVE-w-notes (skip verdicts now one Info line each naming the gate; review corrected the issue's premise 1 — the WARN was already post-gate on main, the every-boot WARNs were pre-#4984 builds) · sdk#121→PR#122 APPROVE-w-notes (**inert until v0.35.2 tag + pin bump**; latent twin in pilot's legacy adapter → #4998) · console#179→PR#181 APPROVE-w-notes · **#4986→PR#4990 pre-merge APPROVE → operator-merged 11:49Z (TASK-479 CLOSED+archived**; graphql scoping verified identical to cli/cli). **The catch: #4987→PR#4992 APPROVE-w-DEFECTS — merged Jira done leg is DEAD CODE** (sdk adapter drops `OnPRCreated`; reconciler adopts `pilot/GH-*` only; external-merge path never calls it; pinned sdk transitions English-only) — third TASK-460 incident, second this week; evidence rows + "reachability evidence" leg added to TASK-460, pitfall `merged-feature-dead-callback-not-bridged-onprcreated`, follow-ups **#4999 + sdk#123/#124**. Also: autopilot-fix spawner fired 45s AFTER its origin issue was delivered (#4995, closed moot) → **#4997** (spawn gate: skip when origin PR closed/issue delivered). #4998 filed (legacy jira adapter ADF twin — live in webhook path). Queue on close: #4997/#4998/#4999 + sdk#123/#124 pilot-labeled. Marker: `2026-08-19_postmerge-review-batch.md` (updated).
- **08-19 (morning): post-merge review batch ×7 DONE + a merged-but-broken fix caught + #4982 narrative corrected + 8 follow-ups filed.** All verdicts posted on the PRs: #4977 (no_op contract) APPROVE-w-notes · **#4978 (Linear UUID) APPROVE-w-DEFECTS — functional no-op**, `$teamId: String!` in an `ID` position, live api.linear.app rejects the merged document pre-auth; #4884 symptom persists → **#4985** filed (one-token `ID!` + declaration-pinning test; pitfall `graphql-mock-tests-cannot-catch-schema-validation`) · #4979 (StripIssuePrefix) APPROVE-w-notes · #4980 (signal-strip consolidation) APPROVE-w-notes (telegram's 7 extra v1 markers silently dropped — net improvement, undocumented) · #4981 (GH_REPO/GH_HOST) APPROVE-w-notes · #4984 (train recovery) APPROVE-w-defects · console PR#178 (repo sync) APPROVE-w-notes (2 MEDIUMs → console#179 filed + #180 design-needed). **Box verification rewrote #4982**: the "missed" 14:00Z tick RAN (scope row done 14:00:05Z, adopted v2.261.0); recovery fired at 15:11Z and was correctly suppressed by the zero-logging row-exists return — defect #2 never existed, defect #1 (early re-fire of yesterday's covered tick) confirmed fixed by PR#4984's last-release gate; log-hygiene fix → **#4989** (memory `gh4982-defect2-was-silent-skip-misread`). **KAN-6 watch item closed**: Jira merge-side close leg **doesn't exist** — card stuck in To Do with pilot-done label; start-notify WARNs on a landed comment (SDK `Comment.Body string` vs Cloud ADF) and name-based transitions can't work on localized workflows → **#4987** (pilot done-leg, category-matched transitions) + **sdk#121** (ADFText for comments). Stale-diff-base intent-judge vetoes: 5th occurrence in 2 days → **#4988** filed (fresh merge-base at judge time). ghguard residuals → **#4986** (D2 graphql+fields + `--hostname` twin). TASK-480 archived; TASK-479/481 pending their follow-up merges. Marker: `2026-08-19_postmerge-review-batch.md`.
