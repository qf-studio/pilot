...
| **P1** | **Hosted retry path for failed executions** ([TASK-485](tasks/TASK-485-hosted-retry-path-failed-executions.md), [#5008](https://github.com/qf-studio/pilot/issues/5008)) | Founder scoping answered 2026-08-25 — UNBLOCKED. Research correction (mem-176): wedge is the identical-failure streak (threshold 2 → `stalled` + `pilot-blocked`), NOT terminal dedupe. 3 legs: env-class failure classifier (daemon) → stalled re-arm probe extending GH-5139 (daemon) → C8 label-cycle supersede (console repo). Live repro fixture: ship-test-js#6. |
