...
**Recent (v2.166.8, June 2 2026):** TASK-356 daemon-loop fixes surfaced by the SDK board-loop run.
...
- **#2 board write-back (v2.166.8, PR #3391):** externally-merged PRs (manual `gh pr merge`, forced by the stage approval-misconfig) never hit `handleMerging`, so cards stuck "In Review". `ScanRecentlyMergedPRs` now moves the card to **Done** (resolves issue node ID + `UpdateProjectItemStatus`, idempotent). **#2-A** "degrade approval gate" = deliberate **no-code** (policy/config); **#3** no-status PR cards = **config-only** (GitHub Project auto-add workflow — not our code). Follow-ups: decouple board sync from on_merge release; #1 auto-recover the stranded branch. Plan: `.agent/tasks/TASK-356-epic-decompose-workloss-and-loop-findings.md`.
