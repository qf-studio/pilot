...
| **P1** | **pilot-docs → GitHub + AWS deploy** ([TASK-492](tasks/TASK-492-pilot-docs-gitlab-to-github-migration.md)) | **Step 1 COMPLETE + verified e2e 09-05** (repo mirrored · GHCR build · dual-push sync #5312/PR#5313 reviewed · same `prod-*` tag on both remotes). **Founder: add `workflow` scope to the token before Step 2** · Step 2 deploy leg blocked on Nelya's answers (Slack `#infrastructure`) · Step 3 cutover · Step 4 retire GitLab (`quant-flow/pilot-docs`, `devops` runner, registry, dead `gitlab:` config block, archive space-reclaim SOP). |
