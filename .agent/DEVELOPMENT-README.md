...
**Current Version:** box build `v2.243.0-8-g0c82048b` (deployed 07-20 21:47Z from main; release v2.243.0 cut by train 07-20 14:03Z; next train 07-21 16:00 Berlin, armed) — **daemon runs on AWS** (`i-0e0c1ca34e7b561f9`, TASK-409; ops via `pilot-aws` skill; NO local daemon; binary at `/var/lib/pilot/bin/pilot`, `/usr/local/bin/pilot` is a symlink) — **⚡ approvals OFF since 07-20 21:41Z**: `stage.require_approval: false` globally, auto-merge on green CI (decision `approvals-off-stage-auto-merge`; escalation gates remain) | full status in `.agent/system/FEATURE-MATRIX.md`
