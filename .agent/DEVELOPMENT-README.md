...
| **P0** | **Irreversible-action audit** ([TASK-459](tasks/TASK-459-irreversible-action-audit.md)) | **Phases 1–3 ✅ merged** (P1 #4796→PR#4802 inventory + `Verdict` · P2 #4811→PR#4812 CI-failure gate, live on box · P3 #4817→PR#4821 recorded-status-authoritative, merged 08-09, **post-merge reviewed 08-10: no defects**). **Phase 4 DISPATCHED 08-10 → [#4823](https://github.com/qf-studio/pilot/issues/4823)** (final leg: evidence-derivation parity test · approval-source vocabulary ×3→1 · `approval_source: ""` overlay guard · §9 reason-string targets · `check-destructive-calls.sh` grep gate · SOP · TASK-460 inventory hook; first parity target found already shipped via GH-4779 at re-anchor). Phase 5 resolved out 08-08: false-success class split to TASK-460. The false-signal class: irreversible actions (close PR, delete branch, spawn fix issue, burn retry) taken on inferred failure at log-line confidence. Three root patterns — absence-of-evidence read as failure · failure inferred from side-effects instead of recorded status · one fact implemented twice and drifting. 08-06/07 cost: a correct PR closed 3× (~$10.65 discarded), a resurrected PR re-closed in 90s, 3 junk fix-issues, 1 false alert. |
