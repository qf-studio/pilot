...
- **Unreleased on `main`:** the TASK-370 comms-factory refactor (PR [#3646](https://github.com/qf-studio/pilot/pull/3646)) merged *after* the v2.192.1 tag — will roll into the next daemon release or a manual `make release V=2.193.0`. No-op until the LLM classifier is enabled, so no functional impact.
