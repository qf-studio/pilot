...
**Recent (Aug 4 – Aug 14 2026; detail lives in `system/saas-roadmap.md`, `system/approval-architecture-roadmap.md`, `tasks/archive/`, and git log — do not re-grow this block, replace it):**
- **08-14 (design → execution, one day): morning design sprint ×4 pages (connections/instances/onboarding/settings v1 — 13 screens, rail 100% designed) → [TASK-478](tasks/TASK-478-console-rail-implementation.md) console-rail program planned (17 legs) → 12 of 17 MERGED + REVIEWED same day, all APPROVE** (console PR#119/121/123/125 · ui PR#51→65). Two pre-S3.3 adapter drift classes killed (instances DTO/routes · connections `{label,secret,config}`/`{valid,error}`). **Slack approval path proven e2e ×2 live** (PR#63/#65: size-hold → Slack approve → applied → autopilot merge → journal closed). Filed [pilot#4869](https://github.com/qf-studio/pilot/issues/4869): externally-merged held PRs never close the execution journal → history rows stuck at `awaiting_approval` (GH-54/GH-58 evidence vs GH-60 comparator). In flight: ui#66 onboarding (guard rework — program risk #5).
...
- **08-06→08-08 (compressed): GitHub Actions outage → recovery → hardening wave; TASK-458 breaker CODE COMPLETE + enabled in prod.** Outage cost: correct PR closed 3× (~$10.65), junk fix-issues, false alert; daemon stopped/restarted, queue drained. Root-cause synthesis → TASK-459 (now fully shipped, see above). Hardening: #4787 structural infra classification · #4790 check-run dedupe · approval B-legs #4776/#4782/#4783/#4795 · #4793 gateway auth (code says GH-4784) · #4800+#4805 superseded≠failed · #4804 prompt guard · breaker #4797+#4806+#4803+#4809 · #4810 asks channel-first. Ops: git identity poisoning fixed. Detail: `system/approval-architecture-roadmap.md` + git log.
- **Earlier (compressed):** 08-04/05 TASK-441 contract hardening + first unattended self-upgrade (v2.253.0) + S4 waves 2–4 + token incident resolved · 08-01/08-03 S4 wave 1 + Golden AMI v2 merged (operator bake pending) · 07-31 first autonomous train + AWS cost audit (`cdk deploy` pending) · 07-30 spec-guard epic + real-stack-verify SOP · 07-29 S3 backend 10/10 · 07-27/28 S2 EXIT MET · 07-26 SaaS UNPARKED · 07-20 approvals off · 07-16 S6-lite AWS cutover (TASK-409). Detail: git log + `tasks/archive/`.
...
| **P1** | **Console rail implementation** ([TASK-478](tasks/TASK-478-console-rail-implementation.md)) | 11 approved designs → shipped surfaces. **12 of 17 legs merged+reviewed 08-14, all APPROVE.** Remaining: UI-9 onboarding (ui#66 in flight, guard-rework review carefully) → UI-10 settings → UI-11 issue page → UI-12 mobile · CON-5 billing portal (founder Stripe gate) · real-stack verify batch UI-2..8 (all fixtures source-derived). |
