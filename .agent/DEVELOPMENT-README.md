...
| **P1** | **Paddle Billing integration** ([TASK-497](tasks/TASK-497-paddle-billing-integration.md)) | 📋 **Design DONE 09-12** (research digests `research/paddle-billing-docs/01…05`; SDK facts re-verified from source). Seven legs L1→L2→(L3∥L4)→L5→L6→L7; **L1 + L3 MERGED and reviewed 09-12** (console PR#298 → #299 → PR#301 APPROVE-w-notes; ui PR#157 → #158 → PR#159 REQUEST-CHANGES → ui#160 → PR#161 APPROVE); **L2 (#300) unblocked**; L4/L5/L6 next. Sandbox smoke + flag-on gated on founder inputs: **sandbox account + API key + client token** (then `/plugin configure paddle@claude-community`; catalog + notification destination get created via the Paddle MCP) · dunning end action · live verification needs domain + Terms/Refund/Privacy pages. Enforcement = reconciler-tick sweep with 72 h past_due grace + `consolectl billing resync`. Contract kept: four `billing_status` values, `{url}` redirect, `/me`. Replaces console Stripe scaffolding #60/#71/#72/#73 (stripe-go removed in L1). |
