...
- **`pilot project add` gh wizard (v2.187.1, [#3017](https://github.com/qf-studio/pilot/issues/3017) → PR [#3612](https://github.com/qf-studio/pilot/pull/3612)):** no-flag TTY wizard — gh-auth detect, repo picker, opt-in token seed from `gh auth token`, `--no-wizard` escape, graceful fallback when gh absent. Pilot-built. TASK-282 archived.
- **Board-GraphQL partial-data tolerance (v2.187.0, [#3609](https://github.com/qf-studio/pilot/issues/3609) → PR [#3610](https://github.com/qf-studio/pilot/pull/3610)):** `ExecuteGraphQLTolerant` + typed `PartialGraphQLError` — board pages keep good nodes on per-node `NOT_FOUND`/`FORBIDDEN`; fatal errors still abort. Closes the last TASK-319 carry-over. Pilot-built.
- **Unreleased on `main`:** PR #3638 (Phase 4a) + #3637 (docs archive) merged *after* the v2.192.0 tag — will roll into the next daemon release or a manual `make release V=2.193.0`. Phase 4a is dormant, so no functional impact.
...
**Earlier (v2.179.0–v2.186.9, June 9–15 2026):** TASK-322 security audit CLOSED · decomposition-integrity waves 1+2 · hot-upgrade self-verify on boot · executor SHA-harvest fix · `safeGo` panic-recovery sweep · board-orphan defense-in-depth · ancestor-tag release dedup. Detail in `git log` + `.agent/tasks/archive/`.
