...
**Recent (Aug 25 – Sep 7 2026; detail lives in `system/saas-roadmap.md`, `system/approval-architecture-roadmap.md`, `tasks/archive/`, and git log — do not re-grow this block, replace it):**
- **09-07→09-09: Nelya cut-over answers, first console release, and a three-day Pilot reliability arc — every merged PR reviewed, ~30 follow-ups filed.** Detail: [TASK-495](tasks/TASK-495-fleet-to-be-alignment-nelya.md) (timestamped log). Highlights: Nelya's five ECS questions answered (sender `console@quantflow.studio`; tenant binary = golden AMI only, no S3 bridge); **pilot-console `prod-0.1.0` released 09-08** (first ECS image; no tag ruleset yet — founder item); CloudFront live in front of quantflow.studio + pilot docs since 09-06 (her reply sat unanswered in a DM thread until 09-09; docs `s-maxage` shipped #5404/PR#5405, live `Hit from cloudfront`); auth-service v0.73.0 deployed by Nelya, `TRUSTED_PROXY_CIDRS` posted; studio-sdk v0.38.1/v0.38.2 (poller branch-lookup fix #138/#140, skip-reason #142). **Pilot incidents**: pilot-console #275 made permanently unpickable by the poller's branch lookup (refiled #280, delivered); PR#5356 abandoned at stage `failed` by autopilot after a size-guard hold (merged by hand; redrive #5378/PR#5387); **PR#5380 merged a self-re-arming loop one minute before REQUEST-CHANGES landed → hand revert PR#5388, the train's v2.273.2 tag cancelled+deleted before publish, box never ran it; fix-forward #5381/PR#5393 then #5398/PR#5401 (GraphQL `lastEditedAt`; Timeline `edited` never fires)**; #5346 stuck 33 h because a body-edit re-arm is not evidence for the stalled sweep (label cycle recovered it); two self-inflicted base-presence holds on my own issue bodies (rule: run the path check BEFORE `gh issue create`). Box: v2.273.1 → v2.273.2 (hand tag 09-09 09:07Z) → v2.274.0 (train 09-09 14:16Z). Memories: `poller-branch-lookup-marks-issue-done-from-another-issues-pr`, `ci-fix-size-guard-holds-own-pr-on-one-lint-annotation`, `stalled-rearm-sweep-ignores-body-edit-loops-forever`, `autopilot-failed-stage-revival-is-a-flag-gated-redrive-before-processpr`, `autopilot-meta-footer-ignored-without-autopilot-fix-label`, `hand-merge-leaves-pilot-needs-human-and-stale-body-capture-at-dispatch`, `board-stalled-timestamp-is-created-at-after-self-upgrade-reconciliation`. **Evening 09-09: PR#5410/#5411/#5412 reviewed (w-notes · w-notes · w-defects).** #5411's review surfaced a live regression from PR#5402: `needs_human` is terminal with no re-arm probe, so clearing `pilot-needs-human` no longer re-admits a task (hold comment still says it does) → **#5414**; #5412's new `OnPRCreated` guard skips fix-issue PRs on a borrowed origin branch once the origin PR leaves `activePRs` → **#5413** (must land before the 09-10 train; #5412 itself is not on the box yet) · `declinedCancel` stale-reason leak → **#5415**. Pitfall `terminal-status-without-rearm-probe-kills-operator-recovery`. Marker: `2026-09-09_evening-three-reviews-needs-human-rearm-regression.md`. **09-10 morning: #5413/#5414/#5415 shipped overnight (PR#5416/#5417/#5418) and reviewed** — #5416 **w-defects**: borrowed-branch plumbing landed only on the rate-limit retry path (my spec pointed at that line; primary SDK `OnPRCreated` path still drops fix-issue PRs; reconciler re-homes new PR under a closed origin issue) → **#5421**; #5417 w-notes (HasLabel guard not mutation-tested → **#5420**); #5418 w-notes. Today's 14:00Z train carries #5410–#5418 together. **Console resume state prepared → [TASK-496](tasks/TASK-496-console-resume-s5.md)**: only open console item is #215/PR#222 (B7, conflicting, REQUEST-CHANGES items unimplemented) — **founder picked close+refile → console #282 filed, PR#222 closed unmerged, #215 closed superseded (09-10 08:45Z); review its PR when it lands**; one ready dispatch (usage rollup + GET usage, body drafted + gate-checked in `tasks/drafts/`); infra#36 is NOT Pilot-executable (model refusal on the package). #5420 → PR#5422 merged+reviewed APPROVE (mutation-verified). Usage rollup dispatched → console #283. #5421 → PR#5423 merged+reviewed APPROVE-w-notes (mutation-verified primary path; per-tick GetIssue+Warn on skipped PRs → **#5425**). **Console legs shipped + reviewed same morning**: #282 → **PR#284 merged 10:12Z, APPROVE-w-notes** (no fail-open stop path; follow-ups console#287 config floor/wiring pin/operator note · console#288 reader active-only + full-page guard · pilot#5426 daemon queue active-only filter — the 50-row `LIMIT` can hide a long-running execution) · #283 → **PR#285 REQUEST-CHANGES → draft**: 0018 migration collision with #284 (CI red, size guard refused a fix issue → needs-human) **plus a real defect: daemon counters are lifetime-cumulative and the API sums hourly snapshots (24 polls of $10 → $240/day)** → revision **console#286 shipped 10:41Z (48b8990), APPROVED; PR#285 un-drafted; autopilot RE-ADOPTED the size-guard-held PR on the new head (#5378 redrive proven live) → founder approved → **PR#285 MERGED 10:53:59Z** (#283 done; #286 closed by hand) → UI leg **ui#150** dispatched. console#287 → PR#289 reviewed APPROVE + autopilot-merged (idle-window floor 10m, IdleSleepEnabled wiring pin, README operator note). **Midday sweep (all landed 11:03–11:44Z, all reviewed)**: console#288 → PR#290 APPROVE (reader `active=1`, full-page guard; contract with daemon confirmed both sides) · pilot#5426 → PR#5428 APPROVE (queue `active` param via `ParseBool`, reuses drift-tested memory terminal set, no LIMIT) · pilot#5425 → PR#5427 **w-defects** (registry consumed before registration commits → persist-evict orphans PR; TTL from dispatch; memo never revalidated → **#5430**) · ui#150 → PR#151 **w-defects** (count-based $5 y-scale flattens real cost; missing days dropped/unsorted → **ui#152**) · founder's #5424 docs SEO → PR#5429 APPROVE-w-notes (built + HTML-verified). Second wave 12:22–12:33Z: #5430 → PR#5431 APPROVE-w-notes (re-record on persist-evict, TTL re-stamp at run start, memo recheck every 30 ticks; mutation survivor on the re-stamp wiring → **#5432**) · ui#152 → PR#153 APPROVE-w-defects (fixes real; mock gap is a rolling block → **ui#154**). Queue at 13:00Z: pilot #5432 · ui #154. **Rule for today: every merged PR has a verdict; follow-ups filed same hour.** Queue: pilot #5425 · #5426 · console #286 → #287 → #288. Review each PR as it lands. **Evening 09-10: founder decision — billing via Paddle, not Stripe** (jurisdiction; memory `payment-processor-is-paddle-not-stripe`; Stripe scaffolding #60/#71/#72/#73 is dead-end; next = nav-task a Paddle Billing design). Console dispatch **#291**: consolectl single-writer advisory lock (an ECS rolling deploy runs two consolectl tasks; no lock existed anywhere). **09-11**: console#291 → PR#292 merged 12:35Z, reviewed **APPROVE-w-defects** (lock correct: pinned conn, session try-lock, bounded drain, ordered release; wiring unpinned — dropping `lock.Run` from either entrypoint fails no test; status query lacks db-oid filter) → **console#293**. Queue: console #293 only.
