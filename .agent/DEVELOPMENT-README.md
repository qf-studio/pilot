...
**Current Version:** box runs **v2.273.1** (hot self-upgrade 2026-09-07 14:19Z; `orchestrator.receipts_digest` live, first scheduled delivery verified 16:00Z). Main is 8 PRs ahead (#5363–#5373: re-arm race, subtask paint, size-guard exemption, sdk v0.38.1 pin, review-path gate) — next train 09-08 14:00Z. **pilot-console `prod-0.1.0` released 09-08 10:06Z** (first ECS image). Prior context in the archived markers.
...
- **09-06→07: AWS consolidation day + Pilot reliability arc — ALL UNCLOSED ITEMS in marker `before-compact-2026-09-07-1245_aws-consolidation-unclosed.md`.** Nelya moved website/docs/auth to AWS (TASK-492 cutover, TASK-494 sync fix); fleet TO-BE accepted (TASK-495: ECS console, her CFN over CDK, founder box stays; pilot-console #259–#264/#270/#274/#45 shipped+reviewed; ECS manifest sent); auth-service #506/#507/#508/#512 merged (tag v0.73.0 due 16:00 Berlin → Nelya sets `TRUSTED_PROXY_CIDRS`); #5228 declined by design → TASK-493 v2.273.0 + #5337. Pilot reliability: 4 pitfalls found the hard way — decomposition splits on acceptance bullets (#5350 → PR#5352; its 'ignores no-decompose' claim was wrong — label applied post-pick), fix-issue Depends-on deadlock (#5341 ✅), backend-timeout false no-op (#5342 → PR#5345 partial, #5346 follow-up), CI-fix continuation force-resets branch to main + own close read as external (#5348 → PR#5349 items 1–3, #5351 rest); pilot-console #275 false success reopened → **unpickable (poller branch-lookup, studio-sdk#138) → refiled #280**. Box rebuilt + restarted 09-07; receipts digest live.
