...
**In flight — Conversational Bot Module (latency fix + issue intake):** decomposed from plan `there-is-a-problem-inherited-fiddle.md`. Fast direct-LLM chat/Q&A (no executor) + persona + message→issue intake, built by **extending `comms.Handler`** (not a parallel listener); disabled ⇒ identical-to-today executor fallback. Phases: [TASK-373](tasks/TASK-373-bot-llm-answer-primitive.md) `internal/llm` Answer primitive · [TASK-374](tasks/TASK-374-bot-responder-fast-chat.md) Responder + fast chat path · [TASK-375](tasks/TASK-375-bot-grounded-qa-retrieval.md) grounded Q&A retrieval · [TASK-376](tasks/TASK-376-bot-issue-intake.md) issue intake (create + label `pilot`) · [TASK-377](tasks/TASK-377-bot-persona-docs.md) persona/voice-scaffold/docs. **Dispatch:** P1+P2 → [#3665](https://github.com/qf-studio/pilot/issues/3665) (Pilot auto-decomposed into epic children #3667/#3668/#3669, executing). P3 [#3671](https://github.com/qf-studio/pilot/issues/3671) / P4 [#3672](https://github.com/qf-studio/pilot/issues/3672) `Blocked by: #3665`; P5 [#3673](https://github.com/qf-studio/pilot/issues/3673) `Depends on: #3671,#3672` — all gated via the poller's `hasPendingDependencies` parse, auto-release when deps close.
