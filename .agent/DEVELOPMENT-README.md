...
**Current Version:** v2.200.2 | full status in `.agent/system/FEATURE-MATRIX.md`

**Recent (v2.187.0–v2.194.1, June 16–25 2026):** **Slack `/help` command routing (two-PR seam fix) · operational-intent live-queue answers · comms intent reliability (ghost-SHA guard fix + unified Handler factory) · M7 Phase 4a github scaffolding — all SHIPPED.**
...
**SHIPPED — Conversational Bot Module (P1–P5, v2.194.2+):** fast direct-LLM chat/Q&A/issue-intake built by extending `comms.Handler`; disabled ⇒ identical executor fallback.
- **P1+P2 (internal/llm Answer primitive + Responder + fast chat):** [#3665](https://github.com/qf-studio/pilot/issues/3665) → `internal/llm.Client`, `comms.Responder`, `handleGreeting`/`handleChat` fast path, `BuildResponder` factory.
- **P3 (grounded Q&A retrieval):** [#3671](https://github.com/qf-studio/pilot/issues/3671) → `Responder.Answer()` with bounded file retrieval (keyword extraction + file scoring, fallback to executor when too broad).
- **P4 (conversational issue intake):** [#3691](https://github.com/qf-studio/pilot/issues/3691) → `Responder.DraftIssue()` + `handleIssueIntake`, creates GitHub issue with `pilot` label directly.
- **P5 (persona, voice scaffold, docs):** [#3673](https://github.com/qf-studio/pilot/issues/3673) → persona honored across Chat/Answer/DraftIssue; `bot.voice.enabled` scaffold (VoiceText seam wired); `configs/pilot.example.yaml` bot: block fully commented; pattern mem-038 (fast-path vs executor) registered.
- Architecture: `comms.Handler` is the shared chokepoint; `AllowMessage()` rate-limits all paths; disabling bot is zero-regression (pure fallback). See pattern `.agent/knowledge/memories/patterns/pattern_fast_conversational_path_vs_executor.md`.
