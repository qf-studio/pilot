...
**Recent (July 21–25 2026):**
- **07-24/25 overnight: dispatch-reliability chain — 4 defects found, fixed, released in v2.246.0.** Started from one `CI_FAILED` on GH-4526 and cascaded: (1) **box Claude credentials died for ~2h** — the box's live `~/.claude/.credentials.json` had been copied to the canary instance, one OAuth session on two machines, canary refreshed and stranded the box. Signature was silent everywhere it should have been loud: TUI worked, headless `claude --print` hung to timeout, executor died `exit status 1` with **empty stderr**, while `curl api.anthropic.com` returned a fast 401. Five executions / 45 min burned before the environment was suspected. (2) **[TASK-418](tasks/archive/TASK-418-ci-infra-failure-classification.md)** (#4534/#4535): a GitHub 429+504 outage was read as a code failure — correct PR closed, garbage fix issue spawned, repick cap → `pilot-blocked`. (3) **[TASK-419](tasks/archive/TASK-419-epic-subissue-claim-loss-deadlock.md)** (#4538): epic self-deadlock — on `ErrClaimLost` the epic polled unbounded for a `queued` child only its own blocked worker could run (`epic.go:2566-2579` + GH-4413's no-ceiling queued phase + `hasLiveWorker` being pure map presence with **zero** `delete(d.workers`). Confirmed twice in the wild; GH-436 hung 3h10m. (4) **[TASK-420](tasks/archive/TASK-420-dashboard-panel-truth-divergence.md)** (#4539): queue panel marked live work done while history kept dead work running — neither panel trustworthy alone. (5) **[TASK-421](tasks/archive/TASK-421-repick-counter-counts-non-failures.md)** (#4541): repick cap counted non-failures, so **any task queued behind a >37min task was auto-blocked for waiting its turn**; verified fixed live (`without counting toward repick hard cap`). Box rebuilt from main + restarted 11:59Z, then **v2.246.0** cut by the 14:00Z train and self-upgraded. #4526 re-armed after 20h blocked → shipped as #4542. Also restored 6 indexed memory files deleted by Pilot's own PRs (GH-4387 class, 4th occurrence). **Open**: `required_checks: [test, lint]` leaves the drift gate + secret scan unable to block a merge (pitfall `required-checks-allowlist-makes-other-gates-decorative`).
- **07-24: hosted canary EXECUTES but won't MERGE (S2 exit blocked) + CI infra-failure class found.** Day 1 of the hosted path end-to-end: fleet VPC live (4 CDK stacks via mgmt runner), `consolectl` shipped (console#47→PR#48), **hosted canary tenant provisioned in 99s** (<5min criterion ✅, `i-0decbc0dcf225cf18`) and executing with real tool-use. A 7-layer defect cascade was filed and fixed/dispatched same-day: console#51→PR#52 (repo-clone gap) · pilot#4526 (daemon scaffolds untracked `.agent/` → its own `git_clean` preflight deadlocks) · console#53 (unit runs as root → claude refuses `--dangerously-skip-permissions`, ledger lands at `/.pilot` off the data volume) · zero-credit `ANTHROPIC_API_KEY` + stale `CLAUDE_CODE_OAUTH_TOKEN` 401 loops · infra#20/#21 (first-deploy CDK rot). **🔴 Blocker found 07-24 night**: PRs #103/#105 sit green-CI and unmerged ~80min, and **no PR has ever merged on `pilot-canary-sandbox`** — execution is proven, the merge leg is not; S2 exit evidence unmet (diagnostic query in `system/saas-roadmap.md` v8.7). **CI infra-failure class**: GH-4526's PR #4529 was correct and killed by a GitHub outage (429 checkout + 504 action fetch) that autopilot read as a code failure — closed the PR, spawned garbage fix #4530, burned the repick cap into `pilot-blocked`. [TASK-418](tasks/TASK-418-ci-infra-failure-classification.md) → [#4531](https://github.com/qf-studio/pilot/issues/4531) dispatched (classify infra vs code from job logs, bounded `rerun-failed-jobs`, `failure_class` metrics); #4528/#4530 closed as superseded; 4526 held `pilot-blocked` until #4531 ships. 5 new pitfalls indexed (187 memories).
- **07-21: GH-4391 shipped after 4 days + v2.244.0 autonomous release + board-sourcing revealed + TASK-410 root cause fixed.** Full-day arc: (1) "2 stuck pointer issues" → root cause: pointer is **board-sourced** (`source_enabled: true` = board #2 REPLACES label discovery, `types.go:42`; card must be in Todo) — pointer#136 wasn't on the board, #129 was already shipped w/ stale card (board status-sync fails INSUFFICIENT_SCOPES). Pitfall `board-sourced-repo-ignores-labeled-issues`; fix #4488→#4489+studio-sdk#104+#4493 bump (SDK-side unsourced-issue WARN+gauge). (2) **TASK-38+39 pointer epics shipped** (~70min dispatch→merged each); **first unattended auto-merges** under approvals-off (#144/#145: verifyCIBeforeMerge→squash→close→branch-delete, zero touch). (3) auth-service had NO test CI (all PRs timed out at waiting_ci) → #441 merged on local test evidence + auth#442 test.yml self-validating PR shipped. (4) **v2.244.0**: 14:00 tick 403'd (pool exhausted at exactly train time) → **#4485 retry succeeded attempt 2** → tag 14:23 → self-upgrade hot-restart 14:29, zero human touch. (5) **GH-4391 root-caused and shipped**: 69 drops ≥ hard cap 5 = designed human-re-arm state (misleading "completed execution exists" = GH-4469 backoff-gate optimization); re-arm = delete `repick_backoff` row → gen-1 claim → PR #4495 (+1482/−89) held by size guard `pilot-needs-human`, CI healed (4 deleted memory docs restored + lint), founder-reviewed (verdict: approve) → merged 17:35. Follow-up #4498 (ETag cache bounds/host-scope). (6) **TASK-410 root cause found+fixed**: "strip unindexed memory doc" pass deletes INDEXED docs, deletions rode #4484 (3 files, unnoticed) + #4489 (2 files) + #4495 branch; filed #4496 → **#4497 auto-merged in ~35min**. Restores: c3413491, 169787ee. (7) Dashboard reconcile-with-store shipped (#4492, rubric-reviewed: project-scoped lookups, no_op card status, gateway-mode 4th gap found beyond plan). (8) State hygiene: 14 stale `autopilot_pr_state` rows purged; carriers 439/443/103/104 remain (real fix = push-main test workflows in auth-service/studio-sdk). **Open watch**: silent one-off daemon death on first `-4` boot; 6 parent issues auto-closed by boot recovery 17:40 unreviewed; CI-watcher timeout clock ticks through rate cooldowns (2 victims 07-21, unfiled by founder choice).
- **07-20 evening: approvals off + 3-bug burn-down + metrics truth live.** Founder flipped `stage.require_approval: false` (auto-merge on green CI, all repos; decision `approvals-off-stage-auto-merge`) — made safe by shipping **#4487 first** (GH-4477: CI re-validated at merge chokepoint, approval rescinded on late check failure). Same evening, all three dispatched fixes merged: **#4485** (GH-4476 release-tick retry w/ 15–30m backoff, 6h window, `release_tick_failed` alert — tomorrow's train is protected), **#4486** (GH-4393 split-brain ledger guard — daemon refuses to open a shadow DB), #4487. **Slack approval flow PROVEN e2e** (#4484 fresh-PR button→merge 19:32Z) — then its send-side twin surfaced: #4485's approval message lost to a websocket drop (pitfall addendum), and the config flip didn't release the held PR (new pitfall `require-approval-flip-doesnt-release-held-prs`; resolution = manual squash-merge). **Per-model metrics fix live** (#4484, box on `v2.243.0-8`): sonnet-5 real success 92.5% (was misread 57%); `no_op`/`infra`/`rate_limited` taxonomy survives hydration. TASK-413 (poller backoff, #4471) + TASK-414 shipped earlier same day; boards live e2e (GH-4472 family); disk rescue root 30→60GB, go caches → data volume.
- **07-19/20 backups live + self-upgrade fixed + loop-class round 2:** v2.242.0 released by train (07-18's tick was lost to a single rate-403 — `scheduleReleaseTick` has NO retry, issue to file). Box self-upgrade to it failed silently at "Creating backup..." — root-owned `/usr/local/bin` vs ec2-user daemon; fixed operationally (binary → `/var/lib/pilot/bin/pilot`, symlink) and in code (**#4470 preflight + ERROR/alert + progress-log throttle**, GH-4468/TASK-412 ✅). **Nightly S3 ledger backups LIVE** (GH-4465→#4466, TASK-411 ✅): 03:31 UTC timer on box, first backup verified in `pilot-s3-agent-data/backups/`, restore SOP `sops/operations/ledger-restore-from-s3.md`; bucket requires `--sse aws:kms`. **GH-4391 poller loop RCA**: 4,233 dispatch→reject cycles over 2 days (major rate-pool drain) — poller ignores `repick_backoff`; TASK-413/GH-4469 filed; its own queued row was then killed overnight by **claim-lost drops counting toward the hard cap** (pitfall `claim-lost-drops-count-toward-hard-cap`) — re-armed, running 09:45Z. #4466's finalize deleted 3 memory files again ("strip unindexed" recipe, GH-4387 3rd hit; drift gate caught it, restored `fa66b810`). New unfiled bug: PR judged green at `handleCIPassed` before a late check fails → daemon holds stale `ci_status=success` at `awaiting_approval` (approval not rescinded). GH-4391 left stalled deliberately — 3 distinct real failures (OOM, size-guard 446>200, heartbeat SIGKILL); split it after 4469 merges.
...
- **Earlier July 13–16 (detail in `tasks/archive/` + git log):** release lane closed end-to-end (#4331→#4337, v2.241.0 first zero-touch cut; manual release = tag push ONLY, mem-159) · ledger-truth reconciliation family (#4368/#4369 hydration heal live-verified, #4370 tag-ancestry backfill) · GH-86 sandbox storm class fixed (claim + no_op-terminal, #4350) · H-track H1–H12 ✅ · mem-ID collision reconciled (mem-159 = release-train pitfall).
...
- **Latest handoff marker**: `.agent/.context-markers/2026-07-24_night_canary-merge-leg-task418.md`
