...
- **07-18/19 (compressed — detail in `git log` + `.agent/tasks/archive/`):** loop-class kill — the three destructive-close rungs in `autopilot/controller.go` replaced by **escalate-and-hold** (GH-4422 epic → #4461/#4462/#4463); repick hard-cap counting restart churn (#4455), RLIMIT_AS→cgroup OOM cap (#4456), ledger completion-recording (#4457), terminal-status writes (#4464); **Slack Socket Mode approvals proven end-to-end** (#4434); pointer waves 2–4 (TASK-12→19, #65–#78). Pitfalls kept: `hard-cap-rearm-in-memory-gate`, `alert-engine-counts-operator-cancels`, `tui-subtask-ghost-rows`.
...
| Priority | Topic | Why |
|----------|-------|-----|
| P1 | **CI infra-failure classification** ([TASK-418](tasks/TASK-418-ci-infra-failure-classification.md)) | 2026-07-24 GH-4526 incident: PR #4529 had all real checks green but lint job died on GitHub infra (429 action download + 504) — autopilot closed the PR, spawned garbage fix issue #4530, and the repick cap put the issue in `pilot-blocked`. Fix: classify infra vs code failure from job logs in `handleCIFailed`, bounded `rerun-failed-jobs` (2/SHA), `failure_class` on CI/PR-failure metrics. Follow-ups (separate issues): repick cap must not count environment failures; auto-close superseded `autopilot-fix` issues. |
