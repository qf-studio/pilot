...
| P1 | ghguard deny residuals ([TASK-479](tasks/TASK-479-ghguard-parser-parity.md) → [#4986](https://github.com/qf-studio/pilot/issues/4986)) | Parser (#4963→PR#4975) + D5 env bypass (#4968→PR#4981) MERGED + reviewed APPROVE-w-notes. Remaining, dispatched as #4986: D2 (`api graphql` with data flags must deny regardless of method) + `--hostname` argv twin of the GH_HOST bypass (policy.go:280 still `actionNone`). Archive TASK-479 when #4986 merges. |
| **P1** | **Leg A retry: Linear UUID filter is a functional no-op** ([TASK-481](tasks/TASK-481-review-followup-defects.md) → [#4985](https://github.com/qf-studio/pilot/issues/4985)) | All three TASK-481 legs merged 08-18 + reviewed 08-19; Legs B/C clean. But PR#4978 declares `$teamId: String!` in an `ID` position — live Linear API rejects the exact merged document pre-auth (`GRAPHQL_VALIDATION_FAILED`), so the #4884 duplicate-label symptom persists. #4985 = one-token `ID!` + pin the declaration in the test. Pitfall: `graphql-mock-tests-cannot-catch-schema-validation`. Close TASK-481 when it merges. |
