...
**Current Version:** v2.194.2 | full status in `.agent/system/FEATURE-MATRIX.md`

**Recent (v2.187.0–v2.198.0, June 16–26 2026):** **Conversational Bot Module (P1–P5, all phases SHIPPED) · Slack `/help` command routing (two-PR seam fix) · operational-intent live-queue answers · comms intent reliability (ghost-SHA guard fix + unified Handler factory) · M7 Phase 4a github scaffolding — all SHIPPED.**
- **Conversational Bot Module — all 5 phases SHIPPED (GH-3665 epic, June 26):** `bot.enabled: true` in YAML routes chat/Q&A/greetings/issue-intake through direct Anthropic API calls (~1-4s) instead of the Claude Code executor (15-30s). Zero-regression fallback: `bot.enabled: false` (or omitted) restores exact executor behavior. Phases shipped:
  - **P1+P2** (TASK-373/374, [#3667](https://github.com/qf-studio/pilot/issues/3667)/[#3668](https://github.com/qf-studio/pilot/issues/3668), v2.196.x): `internal/llm` Answer primitive + `comms.Responder` + fast `handleChat` + `handleGreeting` paths.
  - **P3** (TASK-375, [#3671](https://github.com/qf-studio/pilot/issues/3671), v2.197.0): Grounded Q&A retrieval — bounded file search + LLM; too-broad questions fall back to executor automatically.
  - **P4** (TASK-376, [#3672](https://github.com/qf-studio/pilot/issues/3672), v2.198.0): Conversational issue intake — `DraftIssue` via LLM, creates GH issue labelled "pilot" for auto-execution.
  - **P5** (TASK-377, [#3673](https://github.com/qf-studio/pilot/issues/3673), this PR): Persona applied to all three LLM call sites (`Chat`/`Answer`/`DraftIssue`) via `draftIssueSystemPrompt()`; `bot.voice.enabled` scaffold + `BotVoiceConfig`; `bot.rate_limit` overrides per-adapter limit; `configs/pilot.example.yaml` `bot:` block fully documented; mem-038 pattern registered.
  - Pattern: mem-038 (`patterns/pattern_fast_conversational_vs_executor.md`) — fast path vs executor, persona wiring, voice text flow, test-at-seam rule.
